...
const Mpin = () => {
  ...
  const handleSetPin = () => {
    if (newPin === '' || confirmPin === '') {
      setError(translate('Both fields are required'));
      return;
    }

    if (newPin.length < 4 || newPin.length > 6) {
      setError(translate('PIN must be between 4 and 6 digits'));
      return;
    }

    if (confirmPin.length < 4 || confirmPin.length > 6) {
      setError(translate('Confirm PIN must be between 4 and 6 digits'));
      return;
    }

    // Check if both pins match
    if (newPin !== confirmPin) {
      setError(translate('Pins do not match'));
      return;
    }
    ...
    Alert.alert(
      translate('Success'),
      translate('Your pin has been set successfully!'),
    );
  };

  return (
    <View style={styles.container}>
      <Text style={styles.heading}>{translate('Set_Your_Pin')}</Text>
      <Text style={styles.text}>{translate('key_thispinw_139')}</Text>

      <View style={styles.inputContainer}>
        <TextInput
          style={styles.input}
          placeholder={translate('Enter New Pin')}
        ...
        />
        <TouchableOpacity
          onPress={() => setIsNewPinVisible(prev => !prev)}
      ...
      <TouchableOpacity style={styles.button} onPress={handleSetPin}>
        <Text style={styles.buttonText}>{translate('Set_Pin')}</Text>
      </TouchableOpacity>

      <Text style={styles.footerText}>
        {translate('Make_sure_to_remember_your_pin')}
      </Text>
