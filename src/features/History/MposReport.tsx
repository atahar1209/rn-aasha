...
const SummaryStrip = ({data, isDealer, primary}: any) => {
  const s = useMemo(() => {
    ...
    const getAmount = (t: any) =>
      parseFloat(isDealer ? t.totalamount : t.amount || '0');
    const success = data.filter((t: any) => getStatus(t) === 'SUCCESS');
    return {
      total: data.length,
      success: success.length,
      failed: data.filter(
        (t: any) => getStatus(t) === 'FAILED' || getStatus(t) === 'M_FAILED',
      ).length,
      amount: success
        .reduce((a: number, t: any) => a + getAmount(t), 0)
        .toFixed(0),
    };
  }, [data, isDealer]);
    ...
    <View style={[rc.wrap, {borderLeftColor: st.color}]}>
      <TouchableOpacity onPress={onToggle} activeOpacity={0.85}>
        <View style={rc.topRow}>
          ...
          <View style={{flex: 1}}>
            <Text style={rc.txnId} numberOfLines={1}>
              {translate('Transaction ID')}: {item.TxnId || '—'}
            </Text>
            <Text style={rc.rrn}>
              {translate('RRN')}: {item.rrn || '—'}
      ...
      <View style={rc.footer}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            sharePDF(
              buildRetailerHTML(item),
              `mPOS-${item.TxnId ?? Date.now()}`,
            )
          }
          activeOpacity={0.8}>
    ...
    <View style={[dc.wrap, {borderLeftColor: st.color}]}>
      <TouchableOpacity onPress={onToggle} activeOpacity={0.85}>
        <View style={dc.topRow}>
          ...
          <View style={{alignItems: 'flex-end', gap: 4}}>
            <Text style={[dc.totalAmt, {color: st.color}]}>
        ...
        <View style={dc.chipsRow}>
          {[
            ...
            {
              l: translate('Retailer Comm'),
              v: `₹${item.remincome ?? '0'}`,
              c: '#7C3AED',
              bg: '#F5F3FF',
            },
            {
              l: translate('Total Amount'),
      ...
      <View style={dc.footer}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            sharePDF(buildDealerHTML(item), `mPOS-Dealer-${Date.now()}`)
          }
          activeOpacity={0.8}>
      ...
      {expanded && (
        <View>
          ...
          />
          <InfoRow
            label={translate('Retailer Comm')}
            value={`₹ ${item.remincome ?? '—'}`}
          />
          <InfoRow
  ...
  const fetchTransactions = useCallback(
    async (from: string, to: string, status: string, retailerId: string) => {
      ...
      try {
        ...
        if (IsDealer) {
          const url = `${
            APP_URLS.dealer_Rem_m_possreport
          }ddl_status=${status}&txt_frm_date=${fmt(from)}&txt_to_date=${fmt(
            to,
          )}&allretailer=${retailerId}`;
          data = await get({url});
        } else {
          const url = `${APP_URLS.mposRrport}txt_frm_date=${fmt(
            from,
          )}&txt_to_date=${fmt(to)}&Type=`;
