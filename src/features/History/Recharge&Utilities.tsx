...
const SummaryStrip = ({
  ...
  const stats = useMemo(() => {
    ...
    const total = success.reduce(
      (s, t) => s + parseFloat(t.Recharge_amount || '0'),
      0,
    );
    return {
  ...
  ({
    ...
    return (
      <TouchableOpacity
        ...
        <View style={card.mid}>
          <Text style={card.operator} numberOfLines={1}>
            {item.Operator_name}
          </Text>
          <Text style={card.number}>{item.Recharge_number}</Text>
          <Text style={[card.time, {color: accentColor}]}>
            {item.Reqesttime}
          </Text>
        </View>
