...
const getStatusCfg = (status: string) => {
  ...
  if (status === 'SUCCESS') {
    return {color: '#16A34A', bg: '#DCFCE7', label: translate('Success')};
  }
  if (status === 'FAILED' || status === 'Failed') {
    return {color: '#DC2626', bg: '#FEE2E2', label: translate('Failed')};
  }
  if (status.startsWith('R')) {
    return {color: '#7C3AED', bg: '#EDE9FE', label: translate('Refunded')};
  }
  return {color: '#D97706', bg: '#FEF3C7', label: translate('Pending')};
};
...
const SummaryStrip = ({transactions, isDealer, primaryColor}: any) => {
  const stats = useMemo(() => {
    const getAmt = (t: any) =>
      parseFloat(fv(isDealer, t.amount, t.Amount) || '0');
    ...
    return {
      ...
      failed: transactions.filter((t: any) =>
        ['FAILED', 'Failed'].includes(getStatus(t)),
      ).length,
      amount: success
        .reduce((s: number, t: any) => s + getAmt(t), 0)
        .toFixed(0),
    };
  }, [transactions, isDealer]);

  const chips = [
    {label: translate('Total'), val: stats.total, color: '#374151'},
    {label: translate('Success'), val: stats.success, color: '#16A34A'},
    {label: translate('Failed'), val: stats.failed, color: '#DC2626'},
    {label: translate('Amount'), val: `₹${stats.amount}`, color: primaryColor},
  ];
...
const InfoRow = ({label, value}: {label: string; value: string}) => (
  <View style={ir.wrap}>
    ...
    <Text style={ir.value} numberOfLines={1}>
      {value || '—'}
    </Text>
  </View>
);
...
const BalanceBar = ({pre, debit, post, earn}: any) => (
  <View style={bal.wrap}>
    {[
      {l: translate('Pre Bal'), v: pre, c: '#1D4ED8', bg: '#EFF6FF'},
      {l: translate('Debit'), v: debit, c: '#DC2626', bg: '#FEF2F2'},
      {l: translate('Post Bal'), v: post, c: '#16A34A', bg: '#F0FDF4'},
      {l: translate('My Earn'), v: earn, c: '#7C3AED', bg: '#F5F3FF'},
    ].map(chip => (
        ...
        <TouchableOpacity onPress={onExpand} activeOpacity={0.85}>
          ...
          <View style={txn.topRow}>
            <View style={txn.bankWrap}>
              <View style={[txn.bankIcon, {backgroundColor: st.bg}]}>
                <Text style={[txn.bankInitial, {color: st.color}]}>
                  {(fv(isDealer, item.bank_nm, item.BankName) || 'B')
                    .charAt(0)
                    .toUpperCase()}
                </Text>
              </View>
              <View style={{flex: 1}}>
                <Text style={txn.bankName} numberOfLines={1}>
                  {fv(isDealer, item.bank_nm, item.BankName) ||
                    translate('No Name')}
                </Text>
            ...
            <View style={txn.rightWrap}>
              ...
              <View style={[txn.pill, {backgroundColor: st.bg}]}>
                <Text style={[txn.pillText, {color: st.color}]}>
                  {st.label}
                </Text>
              </View>
            </View>
          ...
          <View style={txn.tagsRow}>
            <View
              style={[
                txn.tag,
                {backgroundColor: isVerify ? '#DCFCE7' : '#FEE2E2'},
              ]}>
              <Text
                style={[
                  txn.tagText,
                  {color: isVerify ? '#16A34A' : '#DC2626'},
                ]}>
                {fv(
                  isDealer,
                  item.Trans_type,
                  isVerify
                    ...
                    : translate('Non-Verified A/C'),
                )}
              </Text>
            </View>
            <Text style={txn.timeText}>
              {fv(isDealer, item.trans_time, item.M_Date)}
            </Text>
          </View>
          ...
          <View style={txn.row2}>
            <View>
              <Text style={txn.micro}>{translate('IFSC Code')}</Text>
              <Text style={txn.micro2}>{item.IFSC || '—'}</Text>
            </View>
            <View style={{alignItems: 'flex-end'}}>
              <Text style={txn.micro}>{translate('Sender')}</Text>
              <Text style={txn.micro2}>
                {fv(isDealer, item.senderno, item.Sender) || '—'}
              </Text>
            </View>
            <View
              style={[
          ...
          <View style={txn.actionBtns}>
            {item.Dmttype === 'DMTN' && (
              <TouchableOpacity onPress={onRefund} activeOpacity={0.8}>
                <Animated.View
                  style={[txn.refundBtn, {backgroundColor: refundAnim}]}>
                  <Text style={txn.refundText}>↩ {translate('Refund')}</Text>
                </Animated.View>
              </TouchableOpacity>
            )}
            <TouchableOpacity
              ...
              <Text style={txn.shareText}>{translate('Share')}</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        {expanded && (
          <View>
            ...
            <InfoRow
              ...
              value={fv(isDealer, item.trans_id, item.TransactionId)}
            />
            <InfoRow
              label={translate('Transaction Mode')}
              value={fv(isDealer, item.Trans_type, item.TransactionType)}
            />
            <InfoRow
              label={translate('Bank RRN')}
              value={item.BankRefId || '—'}
            />
  ...
  const fetchTransactions = useCallback(
    async (from: string, to: string, status: string) => {
      ...
      try {
        ...
        const url = IsDealer
          ? `${
              APP_URLS.dealer_rem_MoneyTransferReport
            }pageindex=1&txt_frm_date=${fmt(from)}&txt_to_date=${fmt(
              to,
            )}&allretailer=&allapiuser=&ddl_status=${status}&ddl_Type=ALL`
          : `Money/api/Money/GetBeneIMPSReport?pageindex=1&pagesize=500&role=Retailer&Id=${userId}&txt_frm_date=${fmt(
              from,
            )}&txt_to_date=${fmt(
              to,
            )}&status=${status}&transtype=ALL&senderno=${searchnumber}`;
    ...
    async (txnId: string) => {
      try {
        if (isotp) {
          ...
          ToastAndroid.show(
            res?.ADDINFO || translate('Done'),
            ToastAndroid.LONG,
          );
        } else {
          const res = await post({
            url: `${APP_URLS.Report_RefundOTP}Txnid=${txnId}`,
          });
          if (res?.status) {
            setShowOtpModal(true);
          } else {
            ToastAndroid.show(
              res?.ADDINFO || translate('Failed'),
              ToastAndroid.LONG,
            );
          }
        }
        ...
        ToastAndroid.show(
          err?.message || translate('Error occurred'),
          ToastAndroid.LONG,
        );
      }
    },
  ...
  const handleShare = useCallback(
    async (item: any) => {
      const rows = [
        ...
        [translate('Status'), fv(IsDealer, item.status, item.Status) || '—'],
        [translate('Amount'), `₹ ${fv(IsDealer, item.amount, item.Amount)}`],
        [translate('IFSC Code'), item.IFSC || '—'],
        [
          translate('Receiver'),
          fv(IsDealer, item.recivername, item.Receiver) || '—',
        ],
        [translate('Bank RRN'), item.BankRefId || '—'],
        [
        ...
        [
          ...
          fv(IsDealer, item.trans_id, item.TransactionId) || '—',
        ],
        [
          translate('Mode'),
          fv(IsDealer, item.Trans_type, item.TransactionType) || '—',
        ],
      ]
        .map(
          ([k, v]) =>
  ...
  `;
      ...
      try {
        ...
        if (!err?.message?.includes('cancel')) {
          Alert.alert(translate('Error'), translate('Could not generate PDF.'));
        }
      }
    },
    [IsDealer],
    ...
    <View style={styles.root}>
      ...
      <DateRangePicker
        ...
        SearchPress={(from: string, to: string, status: string) =>
          fetchTransactions(from, to, status)
        }
        ...
        {loading ? (
          ...
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptySub}>
              {translate('Try changing the date range or status filter.')}
            </Text>
          </View>
        ) : (
          <FlatList
            ...
            keyExtractor={(item, i) =>
              item?.TransactionId ?? item?.trans_id ?? String(i)
            }
