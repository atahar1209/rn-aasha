...
const getStatusCfg = (s: string) => {
  ...
  if (['failed', 'm_failed', 'failure'].includes(st)) {
...
const InfoRow = ({
  ...
  <View
    ...
    <Text style={[card.micro, {textAlign: align}]}>{label}</Text>
    <Text style={[card.micro2, {textAlign: align}]}>{value ?? '—'}</Text>
  </View>
);
      ...
      <TouchableOpacity onPress={onToggle} activeOpacity={0.85}>
        <View style={card.topRow}>
          ...
          <View style={[card.iconWrap, {backgroundColor: st.bg}]}>
            <Text style={{fontSize: wScale(18)}}>🪪</Text>
          </View>

          <View style={{flex: 1}}>
            <Text style={card.name} numberOfLines={1}>
              {translate('Frm_Name')}: {item.Frm_Name || '—'}
            </Text>
            <Text style={card.sub}>
        ...
        {/* ── Mid row: masked PAN + txn type + chevron ── */}
        <View style={card.midRow}>
          <View>
            <Text style={card.micro}>{translate('masked_pan')}</Text>
            <Text style={card.micro2}>{item.masked_pan || '—'}</Text>
          </View>
          <View style={{alignItems: 'flex-end', flex: 1}}>
            <Text style={card.micro}>{translate('transaction_type')}</Text>
            <Text style={card.micro2}>{item.transaction_type || '—'}</Text>
          </View>
          <Text
            style={[
              card.chevron,
      ...
      <View style={[card.banner, {backgroundColor: st.bg}]}>
        <Text style={[card.bannerTxt, {color: st.color}]}>
          {st.label === 'Success'
            ...
            : translate('Transaction Pending')}
        </Text>
      </View>

      {/* ── Footer: txn id + payment method ── */}
      <View style={card.footer}>
        <View>
          <Text style={card.micro}>{translate('transaction_id')}</Text>
          <Text style={card.micro2}>{item.transaction_id || '—'}</Text>
        </View>
        <View style={{alignItems: 'flex-end'}}>
          <Text style={card.micro}>{translate('payment_method')}</Text>
          <Text style={card.micro2}>{item.payment_method || '—'}</Text>
        </View>
      </View>
      ...
      {expanded && (
        <View>
          ...
          <View style={card.balRow}>
            {[
              {
                l: translate('retailer_remain_pre'),
                v: `₹ ${item.retailer_remain_pre ?? '0'}`,
                c: '#1D4ED8',
                bg: '#EFF6FF',
              },
              {
              ...
              {
                l: translate('retailer_remain_post'),
                v: `₹ ${item.retailer_remain_post ?? '0'}`,
                c: '#16A34A',
                bg: '#DCFCE7',
              },
            ].map(b => (
          ...
          <View style={card.balRow}>
            {[
              {
                l: translate('Retailer_gst'),
                v: `₹ ${item.Retailer_gst ?? '0'}`,
                c: '#7C3AED',
                bg: '#F5F3FF',
              },
              {
                l: translate('Retailer_tds'),
                v: `₹ ${item.Retailer_tds ?? '0'}`,
                c: '#DC2626',
                bg: '#FEE2E2',
              },
              {
                l: translate('My_Earn'),
                v: `₹ ${item.Retailer_comm ?? '0'}`,
                c: '#16A34A',
                bg: '#DCFCE7',
              },
            ].map(b => (
...
const PanReport = () => {
  ...
  const recentTransactions = useCallback(
    async (from: string, to: string, status: string, retailerId: string) => {
      ...
      try {
        ...
        const url = `${
          APP_URLS.PanReport
        }ddl_status=${status}&txt_frm_date=${fmt(from)}&txt_to_date=${fmt(to)}`;
        const url2 = `${
          APP_URLS.dealer_Rem_Pancard_report_new
        }ddl_status&txt_frm_date=${fmt(from)}&txt_to_date=${fmt(
          to,
        )}&allretailer=${retailerId}`;
        const response = await get({url: IsDealer ? url2 : url});
        const data = response.Message || [];
        setTransactions(
          IsDealer ? (Array.isArray(response) ? response : []) : data,
        );
      } catch (error) {
  ...
  const renderItem = useCallback(
    ({item, index}: {item: any; index: number}) => {
      const key = item.transaction_id ?? item.rrn ?? String(index);
      return (
        <TxnCard
          item={item}
