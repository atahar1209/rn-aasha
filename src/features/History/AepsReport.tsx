...
const getStatusCfg = (s: string) => {
  ...
  if (FAILED.includes(s))
    return {
      color: '#DC2626',
      bg: '#FEE2E2',
      label: s === 'M_Failed' ? translate('Mini Failed') : translate('Failed'),
    };
  if (s === 'Balance')
    return {color: '#2563EB', bg: '#DBEAFE', label: translate('Bal Enquiry')};
  return {color: '#D97706', bg: '#FEF3C7', label: s || translate('Pending')};
};
...
const buildReceiptHTML = (item: any, isDealer: boolean) => {
  ...
  const rows = [
    [translate('Bank Name'), fv(item.RetailerName, item.BankName) || '—'],
    [
      translate('Account / Aadhaar'),
      fv(item.AccountHolderAadhaar, item.AccountHolderAadhar) || '—',
    ],
    [translate('Status'), getStatusCfg(item.Status).label],
    [translate('Amount'), `${item.Amount ?? 0}`],
    [translate('Transaction ID'), fv('—', item.MerchantTxnId) || '—'],
    [
      translate('Transaction Mode'),
      fv(item.TransactionType, item.TransactionType) || '—',
    ],
    [translate('Bank RRN'), fv(item.BankRRN, item.BankId) || '—'],
    [translate('Mobile No'), item.Mobile || '—'],
    [translate('Request Time'), fv(item.Txn_Date, item.Reqesttime) || '—'],
  ]
    .map(
      ([k, v]) =>
...
const TxnCard = React.memo(
  ({item, isDealer, expanded, onToggle, onShare}: any) => {
    ...
    return (
      <View style={[card.wrap, {borderLeftColor: st.color}]}>
          ...
          <View style={card.topRow}>
            ...
            <View style={{alignItems: 'flex-end', gap: 4}}>
              ...
              <View style={[card.pill, {backgroundColor: st.bg}]}>
                <Text style={[card.pillTxt, {color: st.color}]}>
                  {st.label}
                </Text>
              </View>
            </View>
            ...
            {st.label === 'Bal Enquiry'
              ...
              : `${
                  st.label === 'Success'
                    ...
                    ? '✕'
                    : '⏳'
                } ${translate('Your transaction is')} ${st.label}`}
          </Text>
        </View>
        ...
        <View style={card.footer}>
          <View>
            <Text style={card.micro}>{translate('Request Time')}</Text>
          ...
          <TouchableOpacity
            ...
            onPress={onShare}
            activeOpacity={0.8}>
            <Text style={card.shareTxt}>↑ {translate('Share')}</Text>
          </TouchableOpacity>
        </View>
        ...
        {expanded && (
          <View style={card.expanded}>
            ...
            <View style={card.rowPair}>
              <View style={{flex: 1}}>
                <Text style={card.micro}>{translate('TXN ID')}</Text>
              ...
              <>
                ...
                <View style={card.balRow}>
                  {[
                    ...
                    <View
                      key={b.l}
                      style={[card.balChip, {backgroundColor: b.bg}]}>
                      <Text style={[card.balAmt, {color: b.c}]}>
                        ₹{b.v ?? '0'}
                      </Text>
...
const AEPSAdharPayR = () => {
  ...
  const fetchTransactions = useCallback(
    async (from: string, to: string, status: string, id: string) => {
      setLoading(true);
      try {
        const fmt = (d: string) => new Date(d).toISOString().split('T')[0];
        const url = IsDealer
          ? `${APP_URLS.dealer_Rem_AepsReport}txt_frm_date=${fmt(
              from,
            )}&txt_to_date=${fmt(to)}&allretailer=${id}&ddl_status=${status}`
          : `${
              APP_URLS.aepsReport
            }pageindex=1&pagesize=500&userid=${userId}&txt_frm_date=${fmt(
              from,
            )}&txt_to_date=${fmt(
              to,
            )}&ddl_status=${status}&amount=&BankId=&aadhar=&Type=${type}&userserch_acc_mob=${serchMO}`;
  ...
  const handleShare = useCallback(
    async (item: any) => {
      try {
        ...
        const path = `${ReactNativeBlobUtil.fs.dirs.CacheDir}/AEPS-${
          item.MerchantTxnId ?? Date.now()
        }.html`;
    ...
    <View style={styles.root}>
      ...
      <DateRangePicker
        ...
        SearchPress={(from: string, to: string, status: string) =>
          fetchTransactions(from, to, status, 'ALL')
        }
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text style={styles.emptySub}>
              {translate('Try adjusting the date range or filters.')}
            </Text>
          </View>
        ) : (
          <FlatList
