...
const RetailerCard = React.memo(
  ({item, expanded, onToggle, colorConfig, capRef}: any) => {
    ...
    return (
      <ViewShot
        ref={capRef}
        options={{
          fileName: translate('TransactionReciept'),
          format: 'jpg',
          quality: 0.9,
        }}>
        <View style={[rc.wrap, {borderLeftColor: st.color}]}>
            ...
            <View style={rc.topRow}>
              ...
              <View style={{flex: 1}}>
                <Text style={rc.name} numberOfLines={1}>
                  {translate(`${translate('Firm Name')}: ${item.Frm_Name}`)}
                </Text>
                <Text style={rc.sub}>
                  {translate(`${translate('Txn Type')} : ${item.PG_TYPE}`)}
                </Text>
              </View>
              <View style={{alignItems: 'flex-end', gap: 4}}>
                <Text style={[rc.amount, {color: st.color}]}>
                  ₹ {item.amount}
                </Text>
                <View style={[rc.pill, {backgroundColor: st.bg}]}>
                  <Text style={[rc.pillTxt, {color: st.color}]}>
                    {item.status}
                  </Text>
                </View>
              </View>
          ...
          <View style={[rc.banner, {backgroundColor: st.bg}]}>
            <Text style={[rc.bannerTxt, {color: st.color}]}>
              {translate('Your Transaction in Queue or')} {item.status}
            </Text>
          </View>
          ...
          <View style={rc.footer}>
            <View>
              ...
              <Text style={rc.micro2}>
                {new Date(item.f_date).toLocaleString()}
              </Text>
            </View>
            <TouchableOpacity
              style={rc.shareBtn}
          ...
          {expanded && (
            <View>
              ...
              <View style={rc.balRow}>
                {[
                  {
                    l: translate('Pre Balance'),
                    v: `₹ ${item.remainpre ?? '0'}`,
                    c: '#1D4ED8',
                    bg: '#EFF6FF',
                  },
                  {
                    l: translate('Post Balance'),
                    v: `₹ ${item.remainpost ?? '0'}`,
                    c: '#16A34A',
                    bg: '#DCFCE7',
                  },
...
const DealerCard = React.memo(
  ({item, index, colorConfig, loading, post}: any) => {
    const st = getStatusCfg(item.status);
    ...
    const handleChangeStatus = async () => {
      const res = await post({
        url: `${APP_URLS.UPI_Manual_Pending_To_Success}hideupiidres=${
          item.idno
        }&hideupiidrestypes=${act}&txtBankRRN=${
          act === 'APPROVED' ? item.BankRRN : ''
        }&txtcode=${pin}`,
      });
      if (res) {
        ...
        ToastAndroid.show(
          res?.Message || translate('try again '),
          ToastAndroid.LONG,
        );
      }
        ...
        <View style={dc.topRow}>
          ...
          <View style={{flex: 1}}>
            <Text style={dc.name} numberOfLines={1}>
              {translate('Name')}:{' '}
              {item.RetailerName || translate('Not Available')}
            </Text>
            <Text style={dc.sub}>
              {translate('Date')}: {item.txndate || '0 0 0'}
            </Text>
          </View>
          <View style={{alignItems: 'flex-end', gap: 4}}>
            ...
            {textInputVisible && (
              <View style={dc.inputWrap}>
                <TextInput
                  style={[
                    dc.input,
                    {borderColor: colorConfig.primaryButtonColor},
                  ]}
...
const PaymentGReport = () => {
  ...
  const recentTransactions = useCallback(
    async (from: string, to: string, status: string, retailerId: string) => {
      ...
      try {
        ...
        const url = `${
          APP_URLS.PaymentGateway
        }ddlstatus=${status}&pagesize=500&txt_frm_date=${fmt(
          from,
        )}&txt_to_date=${fmt(to)}`;
        const url2 = `${
          APP_URLS.Get_UPI_DealerTransfer_History
        }txt_frm_date=${fmt(from)}&txt_to_date=${fmt(
          to,
        )}&Retailerid=${retailerId}&ddlstatus=${status}`;
        const response = await post({url: IsDealer ? url2 : url});
        if (response) {
          setTransactions(
            IsDealer ? response.Upitxn_Details ?? [] : response.RESULT ?? [],
          );
        } else {
          setTransactions([]);
        }
  ...
  const renderDealerItem = useCallback(
    ({item, index}: {item: any; index: number}) => (
      <DealerCard
        item={item}
        index={index}
  ...
  return (
    <View style={styles.root}>
      ...
      <AppBarSecond
        title={IsDealer ? 'Add Money Report' : 'Payment Gateway History'}
      />
        ...
        {loading ? (
          ...
          <FlashList
            ...
            keyExtractor={(item: any) =>
              item.idno?.toString() ?? String(Math.random())
            }
