...
const StatusIcon = ({resolved}: {resolved: string}) => {
  ...
  if (resolved === 'pending') {
    return <PaddingSvg2 size={26} />;
  }
  if (resolved === 'failed') {
    return <FailedSvg size={26} />;
  }
  if (resolved === 'refund') {
...
const InfoRow = ({label, value}: {label: string; value: string}) => (
  <View style={ir.wrap}>
    ...
    <Text style={ir.value}>{value || translate('NULL')}</Text>
  </View>
);
  ...
  return (
    <View style={[card.wrap, {borderColor: color}]}>
      ...
      <View style={[card.header, {backgroundColor: color}]}>
        <View style={{flex: 1}}>
          <Text style={card.headerLabel}>
            {translate('Trans_Request_Insert_Time')}
          </Text>
          <Text style={card.headerValue}>{item.Insertdate}</Text>
        </View>
        <View style={{flex: 1, alignItems: 'flex-end'}}>
          <Text style={card.headerLabel}>
            {translate('Cash_Pickup_Collection_Time')}
          </Text>
      ...
      <View style={card.body}>
        ...
        <InfoRow
          ...
          value={item.transid || 'NULL'}
        />
        <BorderLine height={0.5} width="100%" />
        <InfoRow label={translate('Shop_ID')} value={item.Shop_id || 'NULL'} />
        <BorderLine height={0.5} width="100%" />
        <InfoRow
          label={translate('Client_Name')}
          value={item.Clientname || 'NULL'}
        />
        ...
        <View style={card.balRow}>
          {[
            {
              l: translate('Previous_Balance'),
              v: `₹ ${item.RemainPre ?? 0}`,
              c: '#1D4ED8',
              bg: '#EFF6FF',
            },
            {
              l: translate('Paid_Amount'),
              v: `₹ ${item.Amount ?? 0}`,
              c: color,
              bg,
            },
            {
              l: translate('Remaining_Balance'),
              v: `₹ ${item.RemainPost ?? 0}`,
              c: '#16A34A',
              bg: '#DCFCE7',
            },
        ...
        {resolved === 'success' && (
          <>
            <Text style={card.footerTitle}>
              {translate('PickupCollection_Slip')}
            </Text>
            <View style={card.btnRow}>
              <TouchableOpacity
                ...
                activeOpacity={0.8}>
                <Text style={[card.btnTxt, {color: '#16A34A'}]}>
                  {translate('View')}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                activeOpacity={0.8}>
                <Text style={[card.btnTxt, {color: '#2563EB'}]}>
                  {translate('Share')}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                ...
                activeOpacity={0.8}>
                <Text style={[card.btnTxt, {color: '#7C3AED'}]}>
                  {translate('Download')}
                </Text>
              </TouchableOpacity>
            </View>
...
const RadiantPrepayReport = () => {
  ...
  useEffect(() => {
    fetchData(
      normalizeDate(selectedDate.from),
      normalizeDate(selectedDate.to),
      status,
    );
  }, [selectedDate, status]);

  const fetchData = useCallback(
    async (from: string, to: string, sts: string) => {
      setLoading(true);
      try {
  ...
  const PDFRepordata = useCallback(
    async (requestId: string, action = 'view') => {
      try {
        ...
        if (response?.StatusCode === 200 && response?.Content?.length) {
          ...
          Alert.alert(translate('No data found'));
        }
      } catch (error) {
  ...
  const renderItem = useCallback(
    ({item}: {item: any}) => (
      <TxnCard item={item} colorConfig={colorConfig} onPDF={PDFRepordata} />
    ),
