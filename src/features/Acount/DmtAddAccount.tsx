...
const DmtAddAccount = () => {
  const {colorConfig, Loc_Data, userId} = useSelector(
  ...
  const UpdateRetailerBank = useCallback(
    async id => {
      setisLoading(true);
      try {
        ...
        const response = await post({
          url: `${APP_URLS.UpdateRetailerBank}`,
          data,
        });
  ...
  const uploadDoCx = async (bs64, idno) => {
    ...
    try {
      const response = await fetch(
        `https://${APP_URLS.baseWebUrl}/api/user/Uploadcancelledcheque`,
        {
          method: 'POST',
          headers: {
      ...
      Alert.alert(
        translate('Error'),
        `${translate('Failed to upload')}: ${error.message}`,
      );
    } finally {
      setisLoading(false);
    }
  ...
  useEffect(() => {
    ...
    const fetchBankAccounts = async () => {
      ...
      try {
        ...
        Alert.alert(
          translate('Error'),
          translate('An error occurred while fetching bank accounts'),
        );
      } finally {
        setLoading(false);
      }
  ...
  const InfoRow = ({
    ...
    <View style={infoRow.wrap}>
      ...
      <View style={[infoRow.cell, {alignItems: 'flex-end'}]}>
        ...
        <Text style={[infoRow.value, {textAlign: 'right'}]}>
          {right.value || <DotLoader />}
        </Text>
      </View>
    </View>
  );
    ...
    <View style={[cardS.shell, {borderTopColor: PRIMARY}]}>
      ...
      <View style={[cardS.footer, {backgroundColor: PRIMARY_LIGHT}]}>
        ...
        <TouchableOpacity
          style={[
            cardS.viewBtn,
            {borderColor: PRIMARY, backgroundColor: '#fff'},
          ]}
          onPress={() => {
          ...
          <Text style={[cardS.viewBtnText, {color: PRIMARY}]}>
            {translate('View')}
          </Text>
        </TouchableOpacity>
      </View>
          ...
          <KeyboardAwareScrollView
            ...
            <View style={sheetS.sheet}>
              ...
              <View style={formS.body}>
                <SectionLabel text={translate('Bank Details')} />
                <TouchableOpacity
                ...
                <SectionLabel text={translate('Account Details')} />
                <FlotingInput
                ...
                <SectionLabel text={translate('Address Details')} />
                <FlotingInput
                  label={translate('Branch')}
                  value={branch}
                  editable={acnNumber !== ''}
                  onChangeTextCallback={setBranch}
                  inputstyle={formS.input}
                  labelinputstyle={undefined}
                />
                <FlotingInput
                ...
                {base64Img && (
                  <View style={formS.imgPreviewWrap}>
                    <SectionLabel text={translate('Photo Preview')} />
                    <Image
                      source={{uri: `data:image/png;base64,${base64Img}`}}
                      style={formS.imgPreview}
                      resizeMode="cover"
                    />
                  </View>
                )}
