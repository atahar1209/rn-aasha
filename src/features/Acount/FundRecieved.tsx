...
const FundReceivedReport = () => {
  ...
  const FundRecReport = async (from, to, status, fromm) => {
    ...
    let selectedUrl =
      fromm === 'IsDealer' ? url2 : fromm === 'isAdmin' ? url3 : url;
    ...
    try {
      ...
      if (fromm === 'isAdmin') {
        if (response.Report === 'No Data Found') {
          ToastAndroid.show(translate(response.Report), ToastAndroid.SHORT);
        } else {
  ...
  const AmountChip = ({label, value}: {label: string; value: string}) => (
    <View
      ...
      <Text style={[chip.label, {color: PRIMARY}]}>{label}</Text>
      <Text style={[chip.value, {color: PRIMARY, fontWeight: '700'}]}>
        {value}
      </Text>
    </View>
  );
  ...
  const Card = ({children}: {children: React.ReactNode}) => (
    <View style={[card.shell, {borderLeftColor: PRIMARY}]}>{children}</View>
  );
  ...
  const renderItemMaster = ({item, index}) => (
    <Card>
      ...
      <View style={card.amountRow}>
        <AmountChip
          label={translate('Pre Balance')}
          value={`₹ ${item.dealer_prebal || '0'}`}
        />
        <AmountChip
          label={translate('Post Balance')}
          value={`₹ ${item.dealer_postbal || '0'}`}
        />
        <AmountChip
