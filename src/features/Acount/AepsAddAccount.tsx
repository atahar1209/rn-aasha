...
const AepsAddAccount = () => {
  ...
  const uploadDoCx = async (bs64: string, idno: string) => {
    setisLoading(true);
    try {
      const response = await fetch(
        `http://${APP_URLS.baseWebUrl}/api/user/Uploadcancelledcheque`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        throw new Error(translate('Network response was not ok'));
      }
      const responseData = await response.json();
      if (responseData.Message === translate('Image Updated Successfully.')) {
        Alert.alert(
          translate('Success'),
          translate('Image Updated Successfully.'),
        );
      } else {
        Alert.alert(
          translate('Error'),
          responseData.Message || translate('Upload failed'),
        );
      }
    } catch (error: any) {
      ...
      Alert.alert(
        translate('Error'),
        `${translate('Failed to upload')}: ${msg}`,
      );
    } finally {
      setisLoading(false);
    }
  ...
  const fetchBanks = async () => {
    ...
    try {
      ...
      if (response.RESULT === '0') {
        ...
        Alert.alert(translate('Error'), translate('Failed to load bank list'));
      }
    } catch (error) {
      Alert.alert(
        translate('Error'),
        translate('An error occurred while fetching banks'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const sendotp = async () => {
    Keyboard.dismiss();
    if (Addresss === '') {
      Alert.alert(translate('Error'), translate('Please enter the address'));
      return;
    }
    setisLoading(true);
    try {
      ...
      if (res.Status) {
        ...
        Alert.alert(translate('Error'), res.Message);
      }
    } catch (error) {
      console.log(error);
      Alert.alert(
        translate('Error'),
        translate('An error occurred while sending the OTP'),
      );
    } finally {
      setisLoading(false);
    }
  ...
  const submitOtp = async otp => {
    ...
    try {
      ...
      if (
        res?.Message === translate('Otp Miss Match!') &&
        res?.Status === false
      ) {
        Alert.alert(translate('Error'), translate('OTP Miss Match!'));
      } else {
        setIsOtp(false);
        setAdd(false);
        Alert.alert(
          translate('Success'),
          res?.Message || translate('Account added successfully'),
        );
      }
    } catch (error) {
      console.error(error);
      Alert.alert(
        translate('Error'),
        translate('An error occurred while submitting the OTP'),
      );
    } finally {
      setisLoading(false);
    }
  ...
  const DetailRow = ({leftLabel, leftVal, rightLabel, rightVal}: any) => (
    <View style={detailRow.wrap}>
      ...
      <View style={[detailRow.cell, {alignItems: 'flex-end'}]}>
        ...
        <Text style={[detailRow.val, {textAlign: 'right'}]}>
          {rightVal || <DotLoader />}
        </Text>
      </View>
    </View>
  ...
  const VerifyBadge = ({verified}: {verified: boolean}) => (
    <View
      style={[badge.wrap, {backgroundColor: verified ? '#D1FAE5' : '#FEE2E2'}]}>
      <View
        style={[
          badge.icon,
          {backgroundColor: verified ? '#10B981' : '#EF4444'},
        ]}>
        {verified ? <CheckSvg size={12} /> : <CloseSvg size={12} />}
      </View>
      <Text style={[badge.text, {color: verified ? '#065F46' : '#7F1D1D'}]}>
        {verified ? translate('Verified') : translate('Unverified')}
      </Text>
    </View>
  );
  ...
  const StepDot = ({active}: {active: boolean}) => (
    <View
      style={[
        stepDot.dot,
        {backgroundColor: active ? PRIMARY : `${PRIMARY}40`},
      ]}
    />
  );
    ...
    <View style={styles.root}>
      <ScrollView
        ...
        <BottomSheet
          ...
          containerStyle={[
            sheetS.backdrop,
            {paddingBottom: insets.bottom + hScale(10)},
          ]}
          ...
          <KeyboardAwareScrollView
            ...
            <View style={sheetS.sheet}>
              ...
              <View style={sheetS.header}>
                <Text style={sheetS.headerTitle}>
                  {translate('Add_For_Aeps_Ac')}
                </Text>
                <TouchableOpacity
                  style={[sheetS.closeBtn, {backgroundColor: PRIMARY_LIGHT}]}
              ...
              <View style={formS.body}>
                {/* Bank */}
                <Text style={[formS.sectionLabel, {color: PRIMARY}]}>
                ...
                <FlotingInput
                  ...
                  value={ifsccode}
                  editable={bank !== ''}
                  onChangeTextCallback={setIfsccode}
                  inputstyle={formS.input}
                  labelinputstyle={undefined}
                />
                ...
                <FlotingInput
                  ...
                  onChangeTextCallback={setAcnNumber}
                  inputstyle={formS.input}
                  labelinputstyle={undefined}
                />
                ...
                <FlotingInput
                  ...
                  value={Addresss}
                  editable={acnNumber !== ''}
                  onChangeTextCallback={setAddresss}
                  inputstyle={formS.input}
                  labelinputstyle={undefined}
                />
        ...
        <View style={styles.pageBody}>
          ...
          <View
            style={[
              styles.heroBanner,
              {backgroundColor: PRIMARY_LIGHT, borderColor: PRIMARY_MID},
            ]}>
            ...
            <View style={{flex: 1}}>
              <Text style={[styles.heroTitle, {color: PRIMARY}]}>
                {translate('AEPS Account')}
              </Text>
              <Text style={styles.heroSub}>
                {translate(
                  'Aadhaar Enabled Payment System linked bank account',
                )}
              </Text>
            </View>
          </View>
          ...
          <View style={[acctCard.shell, {borderTopColor: PRIMARY}]}>
            ...
            <View style={[acctCard.topBar, {backgroundColor: PRIMARY_LIGHT}]}>
              ...
              <View style={{flex: 1}}>
                <Text
                  style={[acctCard.bankName, {color: PRIMARY}]}
                  numberOfLines={1}>
                  {bankAcclist?.BankName || <DotLoader />}
                </Text>
                <Text style={acctCard.bankSub}>
            ...
            <View style={acctCard.body}>
              <DetailRow
                leftLabel={translate('Account_Holder_Name')}
                leftVal={bankAcclist?.AccountHolder}
                rightLabel={translate('IFSC_Code')}
                rightVal={bankAcclist?.IFSC_CODE}
              />
              <View style={acctCard.divider} />
              <DetailRow
                leftLabel={translate('Account_NO')}
                leftVal={bankAcclist?.AccountNO}
                rightLabel={translate('Branch')}
                rightVal={bankAcclist?.BankAddress}
              />
            </View>
