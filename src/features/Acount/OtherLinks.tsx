...
const DEMO_DATA = [
  {
    Name: translate('Aadhar_Face_Driver'),
    Link: 'https://play.google.com/store/search?q=aadhaar+face+rd',
  },
  {
    Name: translate('Startek_L1'),
    Link: 'https://play.google.com/store/search?q=startek+l1+rd+service',
  },
  {
    Name: translate('Mantra_MFS110_L1'),
    Link: 'https://play.google.com/store/apps/details?id=com.mantra.mfs110.rdservice',
  },
  {
    Name: translate('Morpho_L1'),
    Link: 'https://play.google.com/store/search?q=morpho%20l1',
  },
];

const MASTER_ACCOUNTS = [
  {
    bank_name: translate('ICICI_Bank'),
    link: 'https://buy.icicibank.com/ucj/accounts',
  },
  {
    bank_name: translate('Axis_Bank'),
    link: 'https://www.axisbank.com/retail/accounts',
  },
  {
    bank_name: translate('IDFC_First_Bank'),
    link: 'https://digital.idfcfirstbank.com/apply/savings',
  },
  {
    bank_name: translate('Central_Bank_of_India'),
    link: 'https://vkyc.centralbank.co.in/home',
  },
  {
    bank_name: translate('Union_Bank_of_India'),
    link: 'https://www.unionbankofindia.co.in/english/saving-account.aspx',
  },
  {
    bank_name: translate('HDFC_Bank'),
    link: 'https://applyonline.hdfcbank.com/savings-account',
  },
  {
    bank_name: translate('State_Bank_of_India'),
    link: 'https://sbi.co.in/web/yono/insta-plus',
  },
];

const MASTER_DEMAT = [
  {
    service_provider: translate('Motilal_Oswal'),
    link: 'https://moriseapp.page.link/DoWR7HduvjtkyvWr7',
  },
  {
    service_provider: translate('Angel_One'),
...
const LinkCard = React.memo(
  ({title, url, primaryColor, onPress}: LinkCardProps) => {
    const initial = (title ?? '?').charAt(0).toUpperCase();

    return (
      <View style={styles.card}>
        <View
          style={[styles.avatarBox, {backgroundColor: primaryColor + '18'}]}>
...
const OtherLinks = () => {
  ...
  const openURL = useCallback((url: string) => {
    Linking.openURL(url).catch(() =>
      Alert.alert(translate('Error'), translate('Unable to open link')),
    );
  }, []);
          ...
          renderItem={() => (
            <>
              ...
              {IS_MASTER && list2.length > 0 && (
                <>
                  <SectionLabel title={translate('DEMAT ACCOUNTS')} />
                  <FlatList
                    data={list2}
                    scrollEnabled={false}
