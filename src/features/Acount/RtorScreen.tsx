...
const RtorScreen = () => {
  const {userId, colorConfig, Loc_Data} = useSelector(
  ...
  const data = {
    ...
    data: {
      ...
      Address:
        'D / O Ranjit Singh, Near Dharamshala, Bangi Nihal Singh, Bathinda, Punjab-151301',
    },
  };
  ...
  const FundTransfer = async () => {
    ...
    try {
      ...
      const encryptedPostalCode = encodeURIComponent(
        encryption.encryptedData[11],
      );

      const encryptedLatitude = encodeURIComponent(encryption.encryptedData[7]); // Latitude
      const encryptedLongitude = encodeURIComponent(
      ...
      const encryptedInternetType = encodeURIComponent(
        encryption.encryptedData[7],
      ); // Internet Type (mobile network)
      ...
      console.error(error);
      setisLoading(false);
      Alert.alert(translate('Error'), translate('key_anerroro_7'), [
        {text: translate('OK')},
      ]);
    }
  };
      ...
      <View style={styles.overlay}>
        <View style={styles.dialogContainer}>
          <View style={styles.header}>
            <Text style={styles.title}>{translate('Retailer_Info')}</Text>
          ...
          <View style={styles.content}>
            <DetailRow
              label={translate('Retailer_Name')}
              value={retailerDetails.RetailerName}
            />
            <DetailRow
              label={translate('Firm_Name')}
              value={retailerDetails.Frm_Name}
            />
            <DetailRow
              label={translate('Mobile_No')}
  ...
  return (
    <View style={styles.main}>
      ...
      <View style={styles.container}>
        <FlotingInput
          label={translate('Enter_Retailer_ID')}
          value={id}
        ...
        <FlotingInput
          ...
          labelinputstyle={undefined}
        />
        <DynamicButton title={'Fund Transfer'} onPress={FundTransfer} />
      </View>
