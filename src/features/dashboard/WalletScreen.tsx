...
const WalletScreen = () => {
    ...
    async type => {
      ...
      try {
        ...
        if (resp === 'Success') {
          ...
          Alert.alert(translate(successMsg));
          seteditMode(true);
          return;
        }
        ...
        const errMsg = translate('key_networker_55');
  ...
  const checkUPIStatus = useCallback(async amount => {
    ...
    try {
      ...
      // Extract safely
      const {status: selfStatus, name: selfName, msg: selfMsg} = selfRes || {};
  ...
  const handleModeChange = mode => {
    ...
    if (mode === 'UPI') {
      ...
      mode === translate('Request to Distributor') ||
      mode === translate('Request to Master')
    ) {
      //navigation.navigate("ReqToAdmin", { amount, type: 'Distributor' });

      setPaymentMode(translate('Request to Distributor'));
      setChargeType(translate('Request to Admin'));
      seteditMode(true);
    } else {
  ...
  }, [chargesData, decryptedWalletCharges]);
