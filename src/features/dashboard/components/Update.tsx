...
const UpdateScreen = () => {
  const {colorConfig, logoUrl} = useSelector(
  ...
  const getInstallTime = async () => {
    ...
    Alert.alert(
      translate('App Install Info'),
      `${translate('First Install')}: ${new Date(
        first,
      ).toLocaleString()}\n ${translate('Last Update')}: ${new Date(
        last,
      ).toLocaleString()}`,
    );
  };
  ...
  const handleUpdate = async () => {
    try {
      if (!response) {
        Alert.alert(translate('Error'), translate('Version info not found'));
        return;
      }
      ...
      if (response.isgoogle) {
        ...
        onReceiveNotification2({
          notification: {
            title: translate('Downloading App Update'),
            body: translate('Please Wait'),
          },
        });

        const downloadPath = `${
          ReactNativeBlobUtil.fs.dirs.CacheDir
        }/app-update-${Date.now()}.apk`;
        ...
        ReactNativeBlobUtil.config({
          ...
          .catch(errorMessage => {
            ...
            Alert.alert(
              ...
              translate('Could not download APK.'),
            );
          });
      }
    } catch (error) {
      ...
      <LinearGradient
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.subtitle}>
            {translate(
              'A newer version of the app is available for a better experience.',
            )}
          </Text>
