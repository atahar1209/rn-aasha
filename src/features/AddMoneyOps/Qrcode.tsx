...
const QRCodePage = ({route}) => {
  ...
  const Qrcodestatus = useCallback(
    async amnt => {
      try {
        ...
        if (status === translate('Success') || msz === translate('Create')) {
  ...
  const paymentresp2 = useCallback(
    async txnid => {
      console.log();
      try {
        ...
        console.log(
          'Payment Successful',
          data,
          `${APP_URLS.paytmresp}txnid=${txnid}`,
        );
        if (data === 'Yes') {
          Alert.alert(
        ...
        <View style={styles.contentContainer}>
          <View style={styles.titleContainer}>
            <Text style={styles.title}>
              {mszz === translate('Create')
                ? translate('Create QR CODE')
                : translate('QR CODE')}
            </Text>
          </View>
