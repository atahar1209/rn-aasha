...
const AddMoneyOptions = ({route}) => {
  const {colorConfig, Loc_Data, cmsAddMFrom} = useSelector(
    (state: RootState) => state.userInfo,
  );
  console.log(cmsAddMFrom, '-==-==');

  const color1 = `${colorConfig.secondaryColor}20`;
  const {amount, paymentMode, chargeType, from} = route.params;
      ...
      if (
        ...
        if (amountNum >= minNum && amountNum <= maxNum) {
          ...
          Alert.alert(
            ...
            `${translate(
              'For_UPI_Amount_Should_be_between',
            )} ₹${min} ${translate('to')} ₹${max}`,
            [{text: translate('OK')}],
          );
        }
      } else {
  ...
  }, [from, navigation, amount, get, upich.Minqr, upich.Maxqr, Qrcodestatus]);
  ...
  const showAmountRangeSnackBar = (min, max) => {
    Alert.alert(
      translate('Invalid Amount'),
      `${translate('For_UPI_Amount_Should_be_between')} ₹${min} ${translate(
        'to',
      )} ₹${max}`,
      [{text: translate('OK'), onPress: () => console.log('OK Pressed')}],
      {cancelable: false},
        ...
        if (dealerUpiStatus === 'true') {
          ...
          if (amountt <= dealerBal) {
            if (payAmmot > cappAmount) {
              ...
              Alert.alert(
                translate('Error'),
                translate('Dealer Capping Is Low Please Contact To Admin'),
                [
                  {
                    text: translate('OK'),
                    onPress: () => console.log('OK Pressed'),
            ...
            Alert.alert(
              translate('Error'),
              translate('Dealer Amount Is Low Please Contact To Dealer'),
              [
                {
                  text: translate('OK'),
                  onPress: () => console.log('OK Pressed'),
  ...
  const sendpgresponse = async sendresp => {
    try {
      const url =
        'https://native.vastwebindia.com/PaymentGateway/api/data/Gatewayresponse';
  ...
  const getPaymentMethodValues = (method, charges) => {
    ...
    if (paymentMethod) {
      ...
      setIsLoading(false);
      return ` ${translate('Charge')}: ${cardcharge}\n ${translate(
        'Net Received',
      )}: ${netrecived}\n ${translate('Total')}: ${total}`;
    } else {
      setIsLoading(false);

      return translate('Payment method not found!');
    }
  };
  ...
  const startPayment = async upiUrl => {
    if (!upiUrl) {
      Alert.alert(translate('UPI URL missing'));
      return;
    }
    ...
    try {
      ...
      // ✅ STATUS HANDLING
      if (status === 'SUCCESS') {
        ToastAndroid.show(translate('Payment Successful'), ToastAndroid.SHORT);
        navigation.navigate('AddMoneyPayResponse');
      } else if (status === 'FAILURE') {
        ToastAndroid.show(translate('Payment Failed'), ToastAndroid.SHORT);
        navigation.navigate('AddMoneyPayResponse');
      } else if (status === 'SUBMITTED') {
        ToastAndroid.show(translate('Payment Pending'), ToastAndroid.SHORT);

        navigation.navigate('AddMoneyPayResponse');
      } else {
        ToastAndroid.show(
          translate('Unknown Payment Response'),
          ToastAndroid.SHORT,
        );
      }
    } catch (e) {
      ...
      <View style={styles.container}>
        ...
        <View
          ...
          <Text style={styles.notText}>
            {translate('UPI_Payment_Instruction')}
          </Text>
          ...
          {(cmsAddMFrom === 'CmsPrePay' || cmsAddMFrom === 'PageA') && (
            <>
              <Text style={styles.otherTextS}>
                {translate('Other_Manual_Payment_Options')}
              </Text>
