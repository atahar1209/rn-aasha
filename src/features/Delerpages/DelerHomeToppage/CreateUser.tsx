...
const CreateUser = () => {
  ...
  const validateForm = () => {
    if (!name) {
      ToastAndroid.show(translate('Enter User Name'), ToastAndroid.SHORT);
      return false;
    }
    if (!firm) {
      ToastAndroid.show(translate('Enter Firm Name'), ToastAndroid.SHORT);
      return false;
    }
    if (mobile.length < 10) {
      ToastAndroid.show(
        translate('Enter Valid Mobile Number'),
        ToastAndroid.SHORT,
      );
      return false;
    }
    if (pincode.length < 6) {
      ToastAndroid.show(
        translate('Enter Valid Pincode Number'),
        ToastAndroid.SHORT,
      );
      return false;
    }
    if (!address) {
      ToastAndroid.show(translate('Enter Your Address'), ToastAndroid.SHORT);
      return false;
    }
    if (!stateid) {
      ToastAndroid.show(translate('Select a State'), ToastAndroid.SHORT);
      return false;
    }
    if (!distid) {
      ToastAndroid.show(translate('Select a District'), ToastAndroid.SHORT);
      return false;
    }
    // if (!isValid) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response) {
        Alert.alert(response.status, translate(response.Message));
      } else {
        Alert.alert(
          response.status || '',
          response.Message || translate('Failed to create user'),
        );
      }
      console.log(response);
    } catch (error) {
      Alert.alert(translate('Error'), translate('Failed to create user'));
    } finally {
      setIsSubmit(false);
      setIsLoading(false);
    }
  ...
  function getDistrictId(distName) {
    const district = districtData.find(
      district =>
        district['Dist Name'].toLowerCase() === distName.toLowerCase(),
    );
  ...
  async function PancardCardValidationCheck(pannumber) {
    try {
      const response = await get({
        url: `${APP_URLS.PancardCardValidationCheck}${pannumber}`,
      });
      console.log(response);
      if (response.status === true) {
        setIsValid1(true);
        ...
        setIsValid1(false);
        ToastAndroid.showWithGravity(
          translate('Please Enter Valid PAN number'),
          ToastAndroid.SHORT,
          ToastAndroid.BOTTOM,
        );
      }
    ...
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <BottomSheet
        ...
        <View style={styles.bottomsheetview}>
          <View
            style={[styles.StateTitle, {backgroundColor: color1 || '#f5f5f5'}]}>
            <View style={styles.titleview}>
              <Text
                style={
                  selectbool ? styles.stateTitletext : styles.stateTitletext2
                }>
                {selectbool
            ...
            {selectbool && (
              <TouchableOpacity
                onPress={() => setIsBankVisible(false)}
                activeOpacity={0.7}>
                <ClosseModalSvg2 />
              </TouchableOpacity>
            )}
