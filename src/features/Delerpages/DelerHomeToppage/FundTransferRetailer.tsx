...
const FundTransferRetailer = () => {
  ...
  useEffect(() => {
    ...
    const retailerList = async () => {
      try {
        ...
        console.log(response[0], 'D_CB_status');
  ...
  const UniqueId = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        if (paymentMode === translate('Charge Back') && cbStatus) {
          CBsubmittxn(response.Mecssage), console.log('GDFGDGFG');
        }
        // setIsSubmit(false)
  ...
  const submittxn = async () => {
    ...
    const data = {
      hdMDDLM: encryption.encryptedData[0] || '', // Fallback to empty string if null/undefined
    ...
    try {
      ...
      if (response.Response === translate('Failed')) {
        ...
        Alert.alert(translate(response.Message));
      }
      setIsLoading(false);
      clearAll();
      setIsSubmit(false);
  ...
  const submittxnBRANCH = async () => {
    ...
    try {
      ...
      if (response.Response === translate('Failed')) {
        ToastAndroid.show(translate(response.Message), ToastAndroid.BOTTOM);
        // const decryptedData = {
        //     hdMDDLM: decryptData(encryption.keyEncode, encryption.ivEncode, encryption.encryptedData[0]),
        //     hdPaymentMode: decryptData(encryption.keyEncode, encryption.ivEncode, encryption.encryptedData[1]),
        //     hdPaymentAmount: amount,
  ...
  const CBsubmittxn = useCallback(async transaction => {
    if (!retailerdata?.UserID || !paymentMode || !Subject || !comment) {
      console.log(
        !retailerdata?.UserID || !paymentMode || !Subject || !comment,
      );
      ToastAndroid.show('key_pleasefil_73', ToastAndroid.BOTTOM);
      return;
    }

    if (!amount || isNaN(amount)) {
      ToastAndroid.show('Please provide a valid amount', ToastAndroid.BOTTOM);
      return; // Prevent further execution if amount is invalid
    }
    setIsLoading(true);

    // Proceed with encryption if validation passes
    const encryption = await encrypt([
      retailerdata?.UserID,
      paymentMode,
      Pin,
  ...
  return (
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <KeyboardAwareScrollView
        ...
        extraScrollHeight={130} // Pin और Amount इनपुट को कीबोर्ड से ऊपर रखेगा
        ...
        <View style={styles.container}>
          <TouchableOpacity
            ...
            <FlotingInput
              editable={false}
              label={translate('Select Retailer')}
              value={retailerName}
          ...
          {isSubmit && (
            <FlotingInput
              ...
              value={
                retailerdata.Mobile === null ? '' : `${retailerdata.Mobile}`
              }
              editable={false}
              inputstyle={undefined}
              labelinputstyle={undefined}
          ...
          {isSubmit && (
            <View>
              <FlotingInput
                ...
                keyboardType="number-pad"
                secureTextEntry={secureEntry} // Conditionally hide/show text
                labelinputstyle={undefined}
              />
          ...
          <DynamicButton
            title={
              paymentMode === translate('Charge Back') && cbStatus
                ? translate('Get Otp')
                : isSubmit
        ...
        <BottomSheet animationType="none" isVisible={isVisible}>
          <View style={styles.bottomsheetview}>
            ...
            <TextInput
              placeholder={
                !retailerMode
                  ? translate('Search by Name /Firm name/ Mobile')
                  : translate('Search')
              }
              value={searchQuery}
