...
const RetailerList = () => {
  ...
  const handleSwitchChange = async (newStatus, UserID) => {
    ...
    try {
      const update = await post({
        url: `${APP_URLS.retailerActiveDeactive}RetailerID=${UserID}&Status=${
          newStatus === 'Y' ? 'Y' : 'N'
        }`,
      });
      ...
      console.log(
        update,
        `${APP_URLS.retailerActiveDeactive}RetailerID=${UserID}&Status=${
          newStatus === 'Y' ? 'Y' : 'N'
        }`,
      );
      const response = await post({url: APP_URLS.retailerlist});
      ...
      if (update) {
        ToastAndroid.show(
          translate('User Status Update') + translate(update.Message),
          ToastAndroid.LONG,
        );
      } else {
        ToastAndroid.show(translate(update.Message) || '', ToastAndroid.LONG);
      }
      setLoading(false);
      setLoading1(false);
    } catch (error) {}
  ...
  const renderItem = ({item, index}) => (
    <TouchableOpacity
      style={[styles.card2, {backgroundColor: color1}]}
      onPress={() => {
      ...
      <View
        ...
        {/* <View style={{ alignSelf: 'center' }} >
                    <Text style={styles.status}>{translate("Retailer_Status")}</Text>
        ...
        <View style={styles.detailsSection}>
          ...
          <View
            ...
            <Text style={styles.detailsLabel}>
              {translate('Retailer_Status')}
            </Text>
            <SwitchButton2
              Status={item.Status === 'Y'}
              onChange={v => {
                handleSwitchChange(item.Status, item.UserID);
              }}
            />
          </View>
            ...
            <View>
              ...
              <View style={[styles.row, {}]}>
                ...
                <TouchableOpacity
                  style={styles.btnstyle}
                  onPress={() => {
                    handleCall('location', item.Address);
                  }}>
                  <Entypo
                    name="location"
                    size={30}
    ...
    <ScrollView
      ...
      <View style={styles.main}>
            ...
            <View
              ...
              <TouchableOpacity onPress={() => setModalVisible(false)}>
                <Text
                  style={{
                    fontSize: 18,
                    color: colorConfig.primaryButtonColor,
                    marginBottom: 10,
                  }}>
                  {translate('Close')}
                </Text>
        ...
        <View style={styles.container}>
          <TextInput
            placeholder={translate('Search by name/firm name/ mobile')}
            value={SearchQuery}
