...
const DealerToken = ({navigation}) => {
  ...
  const getData = async () => {
    ...
    console.log(decryptData(data.kkkk, data.vvvv, data), decryptedData);
  ...
  const purchaseToken = async delTok => {
    ...
    try {
      const token_Purchase_Dealer = await post({
        url: `${APP_URLS.token_Purchase_Dealer}${delTok}`,
      });
      console.log(APP_URLS.token_Purchase_Dealer + delTok);
      ...
      Alert.alert(error.message, translate('Failed to purchase token'));
    }
    setIsLoading(false);
  };
  ...
  const [routes] = useState([
    {key: 'Purchased', title: translate('Purchased Token')},
    {key: 'Users', title: translate('Used Token')},
  ]);
