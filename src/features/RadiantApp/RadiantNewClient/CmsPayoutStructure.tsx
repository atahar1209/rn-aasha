...
const Table = ({title, data, showNote}) => {
  ...
  return (
    <View style={styles.tableContainer}>
      ...
      {showNote && (
        <>
          <Text style={styles.notText}>{translate('key_thisappli_170')}</Text>
        ...
        <>
          <View
            ...
            <Text style={[styles.cell, {flex: 2}]}>
              {item.Particular}
              {item.Particular ===
                translate('26 JAN, 15 AUG, 2 OCT, Diwali, Holi') && (
                <Text style={styles.liveText}> {translate('Leave')}</Text>
              )}

              {(item.Particular === translate('All Days Working') ||
                item.Particular === translate('Banking Days Working') ||
                item.Particular === translate('Holiday/Weekend Work')) && (
                <Text style={styles.liveText}>
                  {translate('Emergency_Leave')}
                </Text>
              )}
              {item.Particular ===
                translate('Every Extra Leave / Miss Pickup') && (
                <Text style={styles.liveText}>{translate('Penalty')}</Text>
              )}
            </Text>
            ...
            <Text style={[styles.cell, {flex: 1}]}>
              {item.Prepay}{' '}
              {(item.Particular === translate('Day Collection & By Wallet') ||
                item.Particular ===
                  translate('Evening Collection & By Wallet') ||
                item.Particular === translate('Vaulting & By Wallet')) && (
                <Text style={styles.liveText}>{translate('Per Thousand')}</Text>
              )}
            </Text>
            <BorderLine height={'100%'} width={0.5} />
            ...
            <Text style={[styles.cell, {flex: 1}]}>
              {item.CashDeposit}{' '}
              {(item.Particular ===
                translate('Day Collection & Cash deposit') ||
                item.Particular ===
                  translate('Evening Collection & Cash deposit') ||
                item.Particular === translate('Vaulting & Cash deposit')) && (
                <Text style={styles.liveText}>{translate('Per Thousand')}</Text>
              )}
            </Text>
          </View>
        ...
        {!addInfo ? (
          ...
          <View>
            <View
              ...
              <Text style={[styles.cell, styles.cell2, {flex: 2}]}>
            ...
            <Table
              title={translate('RCE Commission Base Maximum Payout')}
              data={addInfo.CollectionCharges}
              showNote={true}
            />
