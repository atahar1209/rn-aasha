...
const AadharTab = ({
  ...
  const [routes] = React.useState([
    {key: 'front', title: translate('Front Aadhar')},
    {key: 'back', title: translate('Back Aadhar')},
  ]);

  const renderScene = SceneMap({
    front: () => (
      <FrontAadhar imagePath={imagePath} setImagePath={setImagePath} />
    ),
    back: () => (
      <BackAadhar imagePath2={imagePath2} setImagePath2={setImagePath2} />
    ),
  });
        ...
        <View style={[styles.topheader, {backgroundColor: color1}]}>
          ...
          {isUri && (
            <TouchableOpacity onPress={ReUpload}>
              <Text style={styles.donestyle}>{translate('ReUpload')}</Text>
            </TouchableOpacity>
          )}
        </View>
