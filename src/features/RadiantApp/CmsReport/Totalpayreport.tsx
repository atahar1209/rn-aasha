...
const Totalpayreport = ({route}) => {
  ...
  const [onlineRadientAccountnumber, setonlineRadientAccountnumber] =
    useState('');
  ...
  const depositelist = [
    {name: 'BARRELCASHDEPOSIT', label: translate('BARREL CASH DEPOSIT')},
    {name: 'CLIENTCASHDEPOSIT', label: translate('CLIENT CASH DEPOSIT')},
    {name: 'ONLINETRANSFER', label: translate('Online Transfer')},
    {name: 'WalletTransfer', label: translate('Wallet Transfer')},
  ];
  ...
  const Radientcashsubmit1 = async () => {
    ...
    try {
      ...
      if (upperStatus === 'OK') {
        setLoading(false);
        Alert.alert(translate('Success'), responseMessage, [
          {
            text: translate('OK'),
            onPress: () => {
              navigation.goBack();
        ...
        setLoading(false);
        Alert.alert(translate('Pending'), responseMessage, [
          {
            text: translate('OK'),
            onPress: () => {
              navigation.goBack();
        ...
        Alert.alert(
          translate('Failed'),
          responseMessage || translate('Something went wrong!'),
          [
            {
              text: translate('OK'),
              onPress: () => {
  ...
  const validateForm = () => {
    if (!updateamount) {
      ToastAndroid.show(translate('Enter Amount'), ToastAndroid.SHORT);
      return false;
    }
    ...
    switch (stext) {
      case 'BARRELCASHDEPOSIT':
        if (
          !RadientAccountnumber ||
          !currentPreviewImageRef.current ||
          !currentPreviewImageRef.current.startsWith('data:image/')
        ) {
          ToastAndroid.show(
            translate('Fill all Barrel Deposit fields'),
            ToastAndroid.SHORT,
          );
          return false;
        }
      ...
      case 'CLIENTCASHDEPOSIT':
        if (
          ...
          ToastAndroid.show(
            translate('Fill all Client Deposit fields'),
            ToastAndroid.SHORT,
          );
          return false;
        }
      ...
      case 'ONLINETRANSFER':
        if (
          !onlineRadientAccountnumber ||
          !utrnumber ||
          !currentPreviewImageRef.current ||
          !currentPreviewImageRef.current.startsWith('data:image/')
        ) {
          ToastAndroid.show(
            translate('Fill all Online Transfer fields'),
            ToastAndroid.SHORT,
          );
          return false;
        }
      ...
      default:
        ToastAndroid.show(
          translate('Invalid transaction type'),
          ToastAndroid.SHORT,
        );
        return false;
    }
  ...
  const Radientcashsubmit = async () => {
    ...
    try {
      ...
      if (status === 'OK') {
        ...
        Alert.alert(
          translate('Error'),
          parsedData?.Message || translate('Something went wrong!'),
          [
            {
              text: translate('OK'),
              onPress: () => {
          ...
          return (
            <View>
              <TouchableOpacity
                ...
                <Text style={[styles.operatornametext1]}>{item.label}</Text>
              </TouchableOpacity>
            </View>
          );
        }}
  ...
  const showRadientaccount = () => {
    return (
      <FlashList
        data={
          stext === 'BARRELCASHDEPOSIT' || stext === 'ONLINETRANSFER'
            ? response
            : clientresponse
        }
        renderItem={({item}) => {
  ...
  return (
    <View style={styles.main}>
      <AppBarSecond
        title={
          paymentType || PaymentMode === 'GroupPay'
            ? 'Group Payments'
            : 'Individual Payment'
        }
      />
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          <View>
            ...
            />
            {stext === 'WalletTransfer' &&
              walletData.remainbal + walletData.posremain < updateamount && (
                <View style={styles.righticon2}>
                  <Text style={styles.errorText}>
                    {translate('Your_Balance_is_low')}
                  </Text>
                </View>
              )}
          </View>
          ...
          <TouchableOpacity
            ...
            <FlotingInput
              label={translate('Choose Payment Method')}
              value={sLabel}
              editable={false}
              inputstyle={undefined}
              labelinputstyle={undefined}
            ...
            <View>
              ...
              {onlinetransfer && (
                <View>
                  ...
                  <FlotingInput
                    value={utrnumber}
                    label={translate('Enter Utr Number')}
          ...
          {stext !== 'WalletTransfer' && (
            <TouchableOpacity
              ...
              <FlotingInput
                label={
                  currentPreviewImage
                    ? translate('Yes, Slip Uploaded')
                    : translate('UPLOAD SLIP')
                }
                editable={false}
                inputstyle={undefined}
                labelinputstyle={undefined}
        ...
        <Rechargeconfirm
          ...
          details={[
            {label: translate('Payment Mode'), value2: sLabel},
            ...(stext !== 'WalletTransfer'
              ? [{label: translate('Account No.'), value: bankNo}]
              : []),
            ...(stext !== 'WalletTransfer'
              ? [{label: translate('Bank Name'), value2: bName}]
              : []),
          ]}
          lastlabel={translate('Paid Amount')}
      ...
      <BottomSheet animationType="none" isVisible={showRadientList}>
        <View style={styles.bottomsheetview}>
          <View style={[styles.StateTitle, {backgroundColor: color1}]}>
            <View style={styles.titleview}>
              <View>
                <Text style={styles.stateTitletext}>
                  {translate('Select_Account_Number')}
                </Text>
              </View>
            </View>
            <TouchableOpacity
