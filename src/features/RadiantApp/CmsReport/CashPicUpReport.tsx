...
const getStatusCfg = (s: string) => {
  ...
  if ([translate('success'), translate('done')].includes(st)) {
    return {color: '#16A34A', bg: '#DCFCE7'};
  }
  if ([translate('failed'), translate('failure')].includes(st)) {
    return {color: '#DC2626', bg: '#FEE2E2'};
  }
  if ([translate('pending')].includes(st)) {
    ...
    <View style={[card.wrap, {borderLeftColor: cfg.color}]}>
      ...
      <View style={card.topRow}>
        ...
        <View style={{flex: 1}}>
          ...
          <Text style={card.sub}>
            {translate('Date')}: {item.RequestDate || '—'}
          </Text>
        </View>
      ...
      <View style={card.midRow}>
        ...
        <View style={{flex: 1, alignItems: 'flex-end'}}>
          ...
          <Text style={[card.value, {color: cfg.color}]}>
            {item.Status || '—'}
          </Text>
        </View>
      </View>
...
const Walletunloadreport = () => {
  ...
  const recentTransactions = useCallback(
    async (from: string, to: string, status: string) => {
      ...
      try {
        ...
        const url = `${APP_URLS.WalletUnloadReport}txt_frm_date=${fmt(
          from,
        )}&txt_to_date=${fmt(to)}`;
  ...
  const renderItem = useCallback(
    ({item}: {item: any}) => <TxnCard item={item} />,
    [],
  );
