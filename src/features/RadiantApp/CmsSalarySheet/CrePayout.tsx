...
export default function CrePayout() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetchSalarySummary(
          post,
          selectedMonth + 1,
          selectedYear,
        );
    ...
    fetchData();
  }, [post, selectedMonth, selectedYear]);
  ...
  const fetchAllSalary = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetchSalaryDetails(
        post,
        rceId,
        selectedMonth + 1,
        selectedYear,
  ...
  const tableData = salaryData
    ? [
        {
          label1: translate('Pickup/Collection Mode'),
          value1: salaryData?.Pickupmode ?? '-',
          label2: translate('My RCE ID'),
          value2: salaryData?.ceid ?? '-',
        },
        {
          label1: translate('Total Pickup Amount'),
          value1: `₹ ${salaryData?.TotalPickUpAmount ?? 0}`,
          label2: translate('Total Working Day'),
          value2: salaryData?.totalworkingdays ?? '-',
        },
        {
          label1: translate('Commission as Per Pickup'),
          value1: `₹ ${salaryData?.Commission ?? 0}`,
          label2: translate('Minimum Granted Amount'),
          value2: `₹ ${salaryData?.FinalMinPay ?? 0}`,
        },
        {
          label1: translate('Total Travel(KM)'),
          value1: salaryData?.TotalTravels ?? 0,
          label2: translate('Travel Allowance'),
          value2: `₹ ${salaryData?.Maximumpaytravels ?? 0}`,
        },
        {
          label1: translate('Deposit/Activation Charge'),
          value1: `₹ ${salaryData?.Depositecharge ?? 0}`,
          label2: translate('Total Pickup Penalty'),
  ...
  const mapSalaryData = (list = []) =>
    list.map(item => [
      {
        label1: translate('RCE ID'),
        value1: item.ceid,
        label2: translate('Work Mode'),
        value2: item.WorkMode,
      },
      {
        label1: translate('Pickup Amount'),
        value1: `₹ ${item.PickUpAmount}`,
        label2: translate('Commission %'),
        value2: item.Commission,
      },
      {
        label1: translate('Total Commission'),
        value1: `₹ ${item.TotalCommission}`,
        label2: translate('Final Commission'),
        value2: `₹ ${item.FinalCommission}`,
      },
      {
        label1: translate('Minimum Pay'),
        value1: `₹ ${item.minpay}`,
        label2: translate('Final Min Pay'),
        value2: `₹ ${item.FinalMinPay}`,
      },
      {
        label1: translate('Total Working Days'),
        value1: item.TotalWorkingdays,
        label2: translate('Total Days'),
        value2: item.TotalDays,
      },
      {
        label1: translate('Pickup Count'),
        value1: item.Totalpickupcount,
        label2: translate('Zero Pickup'),
      ...
      {
        label1: translate('Penalty'),
        value1: `₹ ${item.TotalPenlity}`,
        label2: translate('Remaining Leave'),
        value2: item.TotalRemainLeave,
      },
      {
        label1: translate('Shop Type'),
        value1: item.ShopType,
        label2: translate('Shop ID'),
        value2: item.Shopid,
      },
      {
        label1: translate('Salary Date'),
  ...
  const travelPenaltyData = allSalary?.TravelsPenalty?.map(item => [
    {
      label1: translate('Total Travel (KM)'),
      value1: item.TotalTravels,
      label2: translate('Max Travel Allowance'),
      value2: `₹ ${item.MaximumpayTravles}`,
    },
    {
      label1: translate('Total Days'),
      value1: item.TotalDays,
      label2: translate('Pickup Count'),
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.note}>{translate('key_noteallt_161')}</Text>
        <View
          style={{
          ...
          <MovingDotBorderText
            ...
            <TouchableOpacity
              ...
              {!tableShow ? (
                <FontAwesome name="eye" color="#000" size={24} />
              ) : (
                <FontAwesome name="eye-slash" color="#000" size={24} />
              )}

              <Text style={styles.viewText}>
                {translate('View_CommissionPayout_Structure')}
              </Text>
            </TouchableOpacity>
          </MovingDotBorderText>
          ...
          <View>
            <View
              ...
              <View>
                <LinearGradient
                  ...
                  <Text style={[styles.sectionTitle, {marginTop: 0}]}>
                    ...
                    {translate(' Monthly Payout Summary')}
                  </Text>
                </LinearGradient>
              </View>
              ...
              <View
                style={[
                  styles.greenValue,
                  showDetails && styles.greenValueTrue,
                ]}>
                <View>
                  <Text style={styles.label}>{translate('Total_PayOut')}</Text>
                ...
                <View>
                  <Text style={styles.label}>{translate('NEt_Balance')}</Text>
                  <Text style={[styles.value]}>₹ {salaryData.NetSalary}</Text>
                </View>
              </View>
                ...
                {allSalary.TravelsPenalty?.length > 0 && (
                  <View
                    ...
                    <Text
                      ...
                      {translate('Travel_Penalty_Details')}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  viewMoreBtn: {
    ...
    backgroundColor: '#fff',
    borderBottomLeftRadius: wScale(8),
    borderBottomRightRadius: wScale(8),
  },
  leftCo: {
    flex: 1,
