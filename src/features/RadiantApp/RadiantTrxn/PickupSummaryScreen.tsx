...
const PickupSummaryScreen = ({route}) => {
  ...
  const {colorConfig, IsRington} = useSelector(
  ...
  useEffect(() => {
    ...
    if (CodeId) {
      ...
      Alert.alert(translate('TXN ID is missing'));
      setIsLoading(false);
    }
  ...
  const defaultContent = [
    {
      ...
      Mobile: '7366007448',
      PhotoName:
        'https://vastbazaar.com/Uploads/4726ac2c-d757-42c7-bb1c-39dfc176ceca139Photomitlesh pic.jpg',
    ...
    return (
      <View style={styles.table}>
        <View style={styles.tableHeader}>
          <Text style={[styles.tableCell, styles.headerCell, styles.col25]}>
            {translate('Denom')}
          </Text>
          <Text style={[styles.tableCell, styles.headerCell, styles.col25]}>
            {translate('No. of Notes')}
          </Text>
          <Text style={[styles.tableCell, styles.headerCell, styles.col50]}>
            {translate('Amount (INR)')}
          </Text>
        </View>
        ...
        <View style={styles.tableRow}>
          <Text style={[styles.tableCell, styles.col25, {fontWeight: 'bold'}]}>
        ...
        <ViewShot
          ...
          <View style={styles.container}>
            <View style={styles.infoSection}>
              <View style={styles.row}>
                <Text style={styles.time}>
                  {entry?.hcislipallow === 'NotAllow' ? 'REQ ID- ' : 'Hci Slip'}
                  <Text style={{color: '#191970'}}>
                    {entry?.hcl_no || '---'}
                  </Text>
                </Text>
                <Text style={[styles.time]}>
                  {translate('Pickup Time')}: {entry?.trans_date || '---'}
                </Text>
              </View>

              <Text style={styles.label}>
                {translate('Customer (Bank) Name')}
              </Text>
              ...
              <Text style={styles.label}>
                {translate('Pickup Point (Name) Address')}
              </Text>
              ...
              <View style={styles.row}>
                <View>
                  <Text style={styles.label}>
                    {translate('Point/Client/Shop ID')}
                  </Text>
                  <Text style={styles.value}>{entry?.shop_id || '---'}</Text>
                </View>
                <View style={styles.rightColum}>
                  <Text style={styles.label}>
                    {translate('Pickup Transaction ID')}
                  </Text>
                  <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    ...
                    <Text
                      style={[styles.value, {marginBottom: 0, marginLeft: 4}]}>
                      {CodeId || '---'}
                    </Text>
                  </View>
                </View>
            ...
            <View style={styles.rceSection}>
              <Image
                source={{uri: entry?.PhotoName}}
                style={styles.avatar}
                resizeMode="stretch"
              />
              <View style={{flex: 1}}>
                <View style={styles.row}>
                  <Text style={[styles.rceText, styles.bold]}>
                    {translate('RCE’s Name')}:{' '}
                  </Text>
            ...
            <View style={styles.amountWords}>
              <Text style={styles.amountWordsText}>
                {translate('Amount in Words')}: {entry?.Amountinwords || '---'}
              </Text>
            </View>
      ...
      <AppBarSecond
        ...
        actionButton={
          <TouchableOpacity style={{alignItems: 'center'}} onPress={onShare}>
            ...
            <Text style={{color: '#fff', fontSize: wScale(10)}}>
              {translate('Share-Slip')}
            </Text>
          </TouchableOpacity>
        }
        onActionPress={() => {}}
