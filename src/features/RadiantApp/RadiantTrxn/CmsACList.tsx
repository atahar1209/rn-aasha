...
const CmsACList = () => {
  ...
  const formatAccountInfo = item =>
    `${translate('Radiant Bank Details')}:\n\n${translate(
      'A/C Name: Radiant Cash Management Services Ltd',
    )}\n${translate('A/C Number')}: ${item.AccountNumber}\n${translate(
      'Bank Name',
    )}: ${item.BankName}\n${translate('IFSC')}: ${item.IfscCode}\n${translate(
      'Branch',
    )}: ${item.BranchName}`;
  ...
  const copyto = item => {
    ...
    ToastAndroid.show(translate('Bank Details Copied'), ToastAndroid.SHORT);
  };
  const copyto2 = item => {
    const message = item.vpaid;
    Clipboard.setString(message);
    ToastAndroid.show(translate('VPA ID Copied'), ToastAndroid.SHORT);
  };
    ...
    Linking.canOpenURL(url)
      .then(supported => {
        if (supported) {
          ...
          ToastAndroid.show(
            translate('WhatsApp is not installed'),
            ToastAndroid.SHORT,
          );
        }
      })
  ...
  const renderField = (label: string, value: string, item?: any) => (
    <View style={styles.rowBox}>
      ...
      <Text
        style={[
          styles.rowValue,
          {fontWeight: item?.AccountNumber && item?.vpaid && 'bold'},
        ]}>
        {value}
      </Text>
    </View>
      ...
      <View style={[{backgroundColor: `${colorConfig.secondaryColor}1A`}]}>
        <View
          ...
          <TouchableOpacity
            ...
            <Text style={styles.sharetxt}>{translate('Share A/C')}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={styles.shareview}
            onPress={() => copyto(item)}>
            <MaterialIcons name="content-copy" size={wScale(20)} />
            <Text style={styles.sharetxt}>{translate('Copy A/C')}</Text>
          </TouchableOpacity>

          {item.BankName === translate('Punjab National Bank') && (
            <TouchableOpacity style={[styles.lotiview]}>
              <LottieView
                autoPlay
                loop
                colorFilters={[{keypath: 'LayerName', color: '#000'}]}
        ...
        <View style={styles.infoContainer}>
          ...
          {renderField(translate('A/C Number'), item.AccountNumber, item)}
          {renderField(translate('IFS Code'), item.IfscCode)}
          {renderField(translate('Bank Name'), item.BankName)}
          {renderField(translate('Branch Info'), item.BranchName)}
        </View>
      </View>
    </View>
  );
      ...
      <View style={[{backgroundColor: '#fff'}]}>
        <View
          ...
          <TouchableOpacity style={styles.shareview} onPress={onShare}>
            <FontAwesome name="whatsapp" size={wScale(20)} color="#075E54" />
            <Text style={styles.sharetxt}>{translate('Share QR')}</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={styles.shareview}
            onPress={() => copyto2(item)}>
            <MaterialIcons name="content-copy" size={wScale(20)} />
            <Text style={styles.sharetxt}>{translate('Copy VPA ID')}</Text>
          </TouchableOpacity>
        </View>
        <ViewShot
          ref={capRef}
        ...
        <View style={styles.infoContainer}>
          {renderField(translate('VPA ID'), item.vpaid, item)}
          {renderField(translate('Bank Name'), item.BankName)}
          {renderField(translate('A/C Number'), item.AccountNumber, item)}
        </View>
      </View>
  ...
  return (
    <View style={styles.main}>
      ...
      <View
        style={{paddingVertical: hScale(10), paddingHorizontal: wScale(10)}}>
        <TabBar
      ...
      <ScrollView>
        <View style={styles.container}>
          <View
            style={[styles.topviw, {borderColor: colorConfig.secondaryColor}]}>
            <Image
              style={styles.toimg}
              source={require('../../../../assets/images/radiant.png')}
            />

            <Text style={styles.toptex}>
              {translate(
                ' Below is the list of accounts available for cash deposit in Radiant Cash Management Services Limited. Please choose the appropriate account as per your convenience. You can choose any branch where you do not have to face the crowd and your time is also not wasted.',
              )}
            </Text>
          </View>
          {loading ? (
            ...
            <FlatList
              data={bankData.filter(item =>
                visible ? item.type === 'online' : item.type === 'qr',
              )}
              renderItem={({item}) =>
