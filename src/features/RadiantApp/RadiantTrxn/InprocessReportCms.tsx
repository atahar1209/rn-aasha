...
const InprocessReportCms = () => {
  ...
  const toggleGroupCheckbox = () => {
    ...
    const selectedItems = newChecked
      ? updatedList.filter(item => item.checked)
      : [];
    ...
    const totalAmount = selectedItems.reduce((sum, item) => {
      const amount =
        parseFloat(
          item.Amountpaid
            ? item.pickup_amount - item.Amountpaid
            : item.pickup_amount,
        ) || 0;
      return sum + amount;
    }, 0);
      ...
      <View>
        <View
          ...
          {item.Reqsts === 'Inprocess' ? (
            <TouchableOpacity
              style={{paddingRight: wScale(7)}}
              onPress={() => handleItemCheckboxToggle(index)}>
              <MaterialIcons
          ...
          <View style={styles.item2}>
            <Text style={[styles.timeLabel]}>
              {translate('Pickup Slip Uploaded Time')}
            </Text>
            <Text style={[styles.timeVabel]}>{item.Insertdate}</Text>
          </View>
        </View>
        ...
        <View
          ...
          <View style={styles.row}>
            <View style={styles.item}>
              <Text style={styles.label}>{translate('HCL SLIP')} -</Text>
            </View>
            <View style={styles.item2}>
          ...
          <View
            ...
            <View style={styles.item}>
              <Text style={styles.label}>{translate('PickUp Amount')}</Text>
              <Text style={styles.value}>{item.pickup_amount}</Text>
            </View>
            <BorderLine height="100%" width={0.3} />

            <View style={styles.item3}>
              <Text style={styles.label}>{translate('Paid Amount')}</Text>
            ...
            <View style={styles.item2}>
              <Text style={styles.label}>{translate('Remain Amount')}</Text>
              <Text style={styles.value}>
                {item.pickup_amount - item.Amountpaid}
              </Text>
            </View>
          ...
          <Text style={styles.currencylabel}>
            {translate('indian currency denominations')}
          </Text>
          <View
            style={[styles.table, {borderColor: colorConfig.secondaryColor}]}>
            ...
            <View style={styles.row2}>
              <Text
                style={[
                  styles.cell,
                  {borderColor: colorConfig.secondaryColor, borderLeftWidth: 0},
                ]}>
                {item.s500}
              </Text>
              <Text
              ...
              <DynamicButton
                ...
                onPress={() => {
                  navigation.navigate('Totalpayreport', {
                    ...
                    selectedAmount: item.Amountpaid
                      ? item.pickup_amount - item.Amountpaid
                      : item.pickup_amount,
          ...
          <View style={{backgroundColor: 'rgba(255,255,255,0.3)'}}>
            <TouchableOpacity
              ...
              <Text style={styles.labelength}>
                {checked
                  ? translate('All Selected Entries')
                  : `${translate('Selected Entries')} ${selectedCount}`}
              </Text>
              ...
              {selectedCount > 0 ? (
                <TouchableOpacity
                  ...
                  <Text style={[styles.paybtntxt]}>
                    {translate('PAY NOW')} ₹ {selectedAmount.toFixed(2)}
                  </Text>
                </TouchableOpacity>
              ) : (
                <Text style={[styles.paybtntxt]}>{translate('Group PAY')}</Text>
              )}
            </TouchableOpacity>
          </View>
        </View>
