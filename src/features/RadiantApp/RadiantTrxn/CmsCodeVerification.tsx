...
const CmsCodeVerification = ({route}) => {
  ...
  const {colorConfig, rctype, rcPrePayAnomut} = useSelector(
  ...
  const copyto = () => {
    ...
    ToastAndroid.show(
      `${translate('scratch Code Copied')}: ${scratchCode}`,
      ToastAndroid.SHORT,
    );
  };
  ...
  const handleVerify = async code => {
    ...
    if (!code) {
      return ToastAndroid.show(
        translate('Scratch Code is required'),
        ToastAndroid.SHORT,
      );
    }
    setIsLoad(true);
    ...
    if (res.Content.ADDINFO.status == 'Done') {
      navigation.navigate('CmsCodeStatus', {
        CodeId,
        item1,
        Mobile: item.Mobile,
        item,
      ...
      ToastAndroid.show(
        translate('Scratch Code') + res.Content.ADDINFO.status,
        ToastAndroid.SHORT,
      );
    } else {
      ToastAndroid.show(
      ...
      <ScrollView keyboardShouldPersistTaps={'handled'} style={{flex: 1}}>
        ...
        <View
          ...
          <View
            ...
            <Text style={styles.label}>{translate('Customer Name')}</Text>
            <Text style={[styles.value, {fontSize: wScale(20)}]}>
              {item1.CustName}
            </Text>

            <Text style={styles.label}>
              {translate('Name / Address / Location')}
            </Text>
            <Text style={styles.value}>{item1.PointName}</Text>
            <View style={styles.rowContainer}>
              <View>
                <Text style={styles.label}>
                  {translate('Point Code/Shop Id')}
                </Text>
            ...
            <View style={styles.rowContainer}>
              <View style={styles.column}>
                <Text style={styles.label}>
                  {translate('Contact Person Name')}
                </Text>
                <Text style={styles.value}>{item.Name}</Text>
              </View>

              <View style={styles.column2}>
                <Text style={styles.label}>{translate('Email ID')}</Text>
          ...
          <View style={styles.importantNote}>
            ...
            <View style={{flex: 1}}>
              <Text style={styles.importantTitle}>
                {translate('Very Important Note')}
              </Text>
              <Text style={styles.importantText}>
                {translate(
                  'Please note that before clicking on the button below (Yes Proceed) select the default or link message way to customer as per your requirement. On proceeding you will get the scratch code on the screen and on the link sent to the customer. Both the parties will exchange the code and enter it in the place where it is to be entered.',
                )}
              </Text>
            </View>
          </View>
          ...
          {!iscode ? (
            <View style={styles.btnCard}>
              <Text style={styles.sectionTitle}>
                ...
                {translate('for successful pickup')}
              </Text>
              <View style={styles.btnRow}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    if (!bluetooth) {
                      setSms(!sms);
                    } // Only toggle if Bluetooth not active
                  }}>
                  <Text style={styles.switchText}>{translate('TEXT SMS')}</Text>
                  {sms ? (
                    <CheckBT size={8} />
                  ) : (
                ...
                <TouchableOpacity
                  ...
                  <Text style={styles.switchText}>{translate('WhatsApp')}</Text>
                  {whatsapp ? (
                    <CheckBT size={8} />
                  ) : (
                    <View style={styles.checkBtn} />
              ...
              <Text style={styles.notTxt}>
                <Text style={{fontWeight: 'bold'}}>{translate('Note')}: </Text>
                {translate(
                  'Click on the slide button from left to right to generate the secret code. Both parties will get a 7 digit code which needs to be shared with each other and submitted.',
                )}
              </Text>
              <TouchableOpacity
                style={[
              ...
              <View
                ...
                <View style={styles.resendRow}>
                  <Text style={styles.resendText}>
                    {translate('click on "Resend Button"')}
                  </Text>
                  <TouchableOpacity
                ...
                <View style={styles.tableRow}>
                  <View style={styles.tableCell}>
                    <Text style={styles.tableLabel}>
                      {translate(
                        ' Your scratch code is below, scratch first and share with customer points',
                      )}
                    </Text>
                  </View>
                  ...
                  <View style={styles.tableCell}>
                    <Text style={[styles.tableLabel, {textAlign: 'right'}]}>
                      {translate(
                        'Ask the Customer Contact Person for a 6 character sketch code and enter it here',
                      )}
                    </Text>
                  </View>
                </View>

                <BorderLine height={0.5} />
                <View style={styles.tableRow}>
                  <View style={styles.tableCell}>
                    <View
                      style={[
                  ...
                  <View style={styles.tableCell}>
                    <TextInput
                      ...
                      placeholder={translate('Enter customer code')}
