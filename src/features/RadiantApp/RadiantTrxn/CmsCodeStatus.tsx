...
export default function CmsCodeStatus({route}) {
  ...
  const {CodeId, item1, item, Mobile, scratchCode, selectedModes} =
    route.params;
    ...
    try {
      ...
      if (
        ...
        if (RCEConfirmation === 'Approved' && ShopConfirmation === 'Approved') {
          ToastAndroid.show(
            translate('Verification Approved'),
            ToastAndroid.SHORT,
          );
          await AsyncStorage.setItem(
            translate('pickup_status'),
          ...
          navigation.navigate('PicUpScreen', {
            item: item1,
            item2: item,
            CodeId: res?.Content?.ADDINFO?.slipcode,
            Mobile,
            selectedModes,
          });
        } else {
          let pendingFields = [];
          if (RCEConfirmation !== 'Approved') {
            pendingFields.push('RCE');
          }
          if (ShopConfirmation !== 'Approved') {
            pendingFields.push('Shop');
          }

          ToastAndroid.show(
            `${translate('Pending')}: ${pendingFields.join(', ')}`,
            ToastAndroid.SHORT,
          );
        }
      } else {
        ToastAndroid.show(
          translate('Unexpected response format!'),
          ToastAndroid.SHORT,
        );
      }
    } catch (error) {
      console.error(translate('Verification Error:'), error);
      ToastAndroid.show(
        translate('Network error. Please try again.'),
        ToastAndroid.SHORT,
      );
    } finally {
      setLoading(false);
      ...
      <ScrollView>
        <View style={styles.container}>
          ...
          <Text
            ...
            {translate('Customer Scratch Code')} - {scratchCode}
          </Text>

          {/* Message */}
          <Text style={styles.message}>
            <Text style={{fontWeight: 'bold'}}>{translate('Sorry')}</Text>{' '}
            {translate(
              'for the inconvenience, but the verification of the scratch code from the customer point has not been completed yet. For security reasons, we cannot allow you to proceed until the verification of the scratch code is complete. Please inform the customer point so that the verification can be completed as soon as possible.',
            )}
          </Text>
          ...
          <View style={styles.statusBox}>
            <View>
              <Text style={styles.statusTitle}>
                {translate('Customer Point Verification')}
              </Text>
              <Text style={styles.statusPending}>
                {translate('S t i l l P e n d i n g')}
              </Text>
            </View>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <Text style={styles.refreshText}>
              {translate('to check the latest status.')}{' '}
            </Text>
          </View>
