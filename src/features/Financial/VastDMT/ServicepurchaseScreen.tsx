...
const ServicepurchaseScreen = ({route}) => {
  ...
  const getCharge = async () => {
    ...
    const url = activeAepsLine
      ? `${APP_URLS.getServiceChargeNifi}Service=${route.params.typename}`
      : `${APP_URLS.getServiceCharge}Service=${route.params.typename}`;
    console.log(url);
    try {
      ...
      Alert.alert(
        translate('Error'),
        error.message || translate('An error occurred'),
        [{text: translate('OK')}],
      );
  ...
  const servicePurchase = async type => {
    try {
      ...
      if (res.Status === 'Success') {
        Alert.alert(translate('Success'), res.Message, [
          {
            text: translate('OK'),
        ...
        Alert.alert(
          translate('Error'),
          translate('There was an issue with your purchase.'),
          [{text: translate('OK')}],
        );
      }
    } catch (error) {
  ...
  return (
    <View style={styles.main}>
      ...
      <View style={styles.container}>
        {visible1 && (
          <View style={styles.bodyview}>
            ...
            <Text style={styles.title}>
              {translate('Your_Service_is_Expired')}
            </Text>
            <Text style={styles.description}>
              {translate('key_yourservi_156')}
            </Text>
            <View style={styles.btnviwe}>
              <DynamicButton
                title={`${route.params.typename} Purchase`}
                onPress={() => {
                  setVisible1(false);
                  setVisible2(true);
                  setTypee('ALL');
        ...
        {visible2 && (
          <View>
            <View style={styles.labelrow}>
              <Text style={[styles.lable, {color: colorConfig.primaryColor}]}>
                {translate('Service_Payment')}
              </Text>
        ...
        <Text style={{marginTop: hScale(20)}}>
