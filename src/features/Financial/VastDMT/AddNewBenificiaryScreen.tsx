...
const AddNewBenificiaryScreen = ({
  ...
  const Radiant = res => {
    if (res.RESULT === '1') {
      Alert.alert(translate('Failed'), res.ADDINFO, [
        {text: 'OK', onPress: () => {}},
      ]);
    } else {
      Alert.alert(translate('Successfully'), res.ADDINFO, [
        {
          text: translate('OK'),
          onPress: () => {
            navigation.goBack();
  ...
  const vast = res => {
    if (res.RESULT === '1') {
      ...
      setGoodMark2(true);
      Alert.alert(translate('Success'), res.ADDINFO.status, [
        {
          text: '',
          onPress: () => {
      ...
      const mockNotification = {
        notification: {
          title: `${
            res.ADDINFO.status === translate('Transaction Successful')
              ? translate('Bank account is linked')
              : ''
          }`,
          body: `${res.ADDINFO.status}- \n${translate('Details')}\n${translate(
            'Name',
          )} :${name}\n${translate('Acc No')} :${accountNumber} \n${translate(
            'Mo',
          )} : ${senderNo}`,
        },
      };
  ...
  const verifyAcc = async (
    ...
    try {
      ...
      if (res.RESULT === '1') {
        ...
        Alert.alert(
          translate('Message'),
          res.ADDINFO || res.ADDINFO.statuscode === 'LOW'
            ? res.ADDINFO.status
        ...
        const mockNotification = {
          notification: {
            title: verify,
            body: `${translate(
              'A/C verified for money transfer status is',
            )} ${verify}, ${translate('name')} : ${bename} `,
          },
        };
        onReceiveNotification2(mockNotification);
  ...
  const ReverifyAcc = async (
    ...
    try {
      const res = await post({
        url: `${APP_URLS.reVerifybankACC}mb=${mb}&iff=${iff}&cd=${cd}&bno=${bno}&mal=${mal}&sk=${sk}&pi=${pi}&kk=${kk}&bankName=${bankName}&kyc=${kyc}&uniqueid=${uniqueid}`,
      });
      if (res.RESULT === '1') {
        setGoodMark(false);
        setIsLoading(false);
  ...
  return (
    <View
      style={[styles.main, {borderColor: `${colorConfig.secondaryColor}30`}]}>
      {isload && <ShowLoader />}
      ...
      <KeyboardAwareScrollView
        ...
        <View style={styles.formContainer}>
              ...
              <View key={i} style={styles.stepItem}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor:
                        i === 0 ? colorConfig.secondaryColor : '#E5E7EB',
                    },
                  ]}>
                  <Text
                    style={[
                ...
                <Text
                  style={[
                    styles.stepLabel,
                    {color: i === 0 ? colorConfig.secondaryColor : '#9CA3AF'},
                  ]}>
                  {step}
                </Text>
              </View>
          ...
          <Text
            style={[styles.sectionLabel, {color: colorConfig.secondaryColor}]}>
            ...
            {validate && (
              <View style={styles.errorBadge}>
                <Text style={styles.errorText}>
                  ⚠ {translate('Account_numbers_do_not_match')}
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.inputCard}>
            <TouchableOpacity
              ...
              <View pointerEvents="none">
                <FlotingInput
                  label={bank || translate('Select Bank')}
                  editable={false}
                  inputstyle={undefined}
                  labelinputstyle={undefined}
          ...
          <View style={styles.actionSection}>
            {isR ? (
              <TouchableOpacity
                ...
                activeOpacity={0.85}>
                <Text style={styles.actionBtnText}>{translate('RADIANT')}</Text>
              </TouchableOpacity>
            ) : (
      ...
      <BottomSheet
        ...
        <View style={styles.bottomSheet}>
          <View
            ...
            <View>
              <Text style={styles.sheetTitle}>
                {translate('Select_Your_Bank')}
              </Text>
              <Text style={styles.sheetSub}>
                {translate('Choose from the list below')}
              </Text>
            </View>
            <TouchableOpacity
          ...
          <View style={styles.searchWrapper}>
            ...
            <TextInput
              placeholder={translate('Search bank')}
              value={searchQuery}
            ...
            {verificationDetails && (
              <View style={styles.modalBody}>
                <View style={styles.detailRow}>
                  ...
                  <View>
                    <Text style={styles.detailLabel}>{translate('Name')}</Text>
                ...
                <View style={[styles.detailRow, {marginTop: hScale(12)}]}>
                  <Text style={styles.detailIcon}>
                    {verificationDetails.status === translate('VERIFIED')
                      ? '✅'
                      : '⚠️'}
                  </Text>
                  <View>
                ...
                <View style={[styles.detailRow, {marginTop: hScale(12)}]}>
                  ...
                  <View>
                    <Text style={styles.detailLabel}>
                      {translate('Bank Reference No')}
                    </Text>
                    <Text style={styles.detailValue}>
                      {verificationDetails.bankRefNo}
                    </Text>
                  </View>
                </View>
              </View>
