...
const IdTypeButton: React.FC<IdBtnProps> = ({
  label,
  active,
  onPress,
  primaryColor,
  ...
  <TouchableOpacity
    ...
    <Text style={[styles.idBtnText, {color: active ? labelColor : '#555'}]}>
      {label}
    </Text>
  </TouchableOpacity>
);
...
const SummaryRow: React.FC<{
  ...
  <View style={styles.summaryRow}>
    ...
    <View style={[styles.summaryCell, styles.summaryCellRight]}>
      <Text style={[styles.summaryLabel, {textAlign: 'right'}]}>
        {rightLabel}
      </Text>
      <Text
        style={[styles.summaryValue, {textAlign: 'right'}]}
        numberOfLines={1}
        ellipsizeMode="head">
        {rightValue}
      </Text>
...
const toBankScreen = ({route}: any) => {
  ...
  const isFormValid = () => {
    ...
    if (transpin.length < 4 || transpin.length > 6) {
      return false;
    }
    return true;
  };
    ...
    async (number: string) => {
      try {
        ...
        if (aadharvis) {
          ...
          message = res?.status
            ? translate('Aadhar_Verified') + ' ✅'
            : translate('Aadhar_Not_Verified') + ' ❌';
        } else if (panvisi) {
          res = await get({
            url: `${APP_URLS.checkUpiSdrAdhar}PancardCardValidationCheck?pannumber=${number}`,
          });
          message = res?.status
            ? translate('Pan_Verified') + ' ✅'
            : translate('Pan_Not_Verified') + ' ❌';
        }
      ...
      try {
        ...
        if (response) {
          ...
          Alert.alert(translate('Payment_Response'), msg, [
            {
              text: translate('Go_To_Dashboard'),
              onPress: () => navigation.navigate('Dashboard'),
            },
          ]);

          onReceiveNotification2({
            notification: {title: translate('Payment_Response'), body: msg},
          });
        } else {
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.inputGroup}>
            <FlotingInput
              ...
              inputstyle={[styles.inputBase, !amountMatch && styles.inputError]}
              value={reamount}
          ...
          <View style={styles.inputGroup}>
            ...
            />
            {transpin.length > 0 &&
              (transpin.length < 4 || transpin.length > 6) && (
                <Text style={styles.errorText}>
                  {translate('PIN_Length_Error')}
                </Text>
              )}
          </View>
          ...
          <TouchableOpacity
            ...
            <LinearGradient
              colors={
                isLoading ? ['#aaa', '#ccc'] : [primaryColor, secondaryColor]
              }
