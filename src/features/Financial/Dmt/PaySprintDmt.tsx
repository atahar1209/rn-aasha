...
const PaysprintDmt = () => {
  ...
  const getDmtPayload = async () => {
    try {
      ...
      Alert.alert(
        translate('Error'),
        translate('Failed to load merchant info'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleNext = async () => {
    if (!merchantCode) {
      Alert.alert(
        translate('Error'),
        translate('Please wait for data to load'),
      );
      return;
    }
    setBtnLoading(true);
    try {
      ...
      if (result === 'CANCELLED') {
        Alert.alert(
          translate('Status'),
          translate('Transaction Cancelled by user'),
        );
      }
    } catch (error: any) {
      Alert.alert(translate('SDK Error'), error.message);
    } finally {
      setBtnLoading(false);
    }
  };
          ...
          {loading ? (
            ...
            <>
              <InfoRow
                ...
                value={maskValue(merchantCode)}
              />
              <InfoRow
                icon="🌐"
                label={translate('Partner ID')}
                value={maskValue(partnerId)}
              />
        ...
        <DynamicButton
          title={
            btnLoading ? (
              ...
              <Text style={styles.btnText}>{'Proceed to Transfer'} →</Text>
            )
          }
          onPress={handleNext}
        />
