...
const QRScanner = () => {
  ...
  const onSuccess = e => {
    setScan(false);
    const data = e.data;
    Alert.alert(translate('QR Code Scanned'), data);
    setUpiDetails({
      ...upiDetails,
      upiId: data,
    });
  ...
  const handleTransfer = () => {
    if (upiDetails.pin === '') {
      Alert.alert(translate('Error'), translate('Please enter PIN'));
      return;
    }
  ...
  const copyToClipboard = text => {
    ...
    Alert.alert(translate('Copied to Clipboard'), text);
  };

  return (
    <View style={styles.container}>
      {scan ? (
        <Text style={styles.centerText}>{translate('Scan_a_QR_Code')}</Text>
      ) : (
        <View style={styles.detailsContainer}>
          <Text style={styles.title}>Paying {upiDetails.upiName}</Text>
          <View style={styles.row}>
            <Text style={styles.label}>{translate('UPI_ID')}</Text>
          ...
          <View style={styles.row}>
            ...
            <TextInput
              ...
              onChangeText={text =>
                setUpiDetails({...upiDetails, amount: text})
              }
            />
          </View>
          <View style={styles.row}>
            <Text style={styles.label}>{translate('Sender Number')}:</Text>
            <TextInput
              style={styles.input}
              keyboardType="phone-pad"
              value={upiDetails.senderNumber}
              onChangeText={text =>
                setUpiDetails({...upiDetails, senderNumber: text})
              }
            />
          </View>
          <View style={styles.row}>
