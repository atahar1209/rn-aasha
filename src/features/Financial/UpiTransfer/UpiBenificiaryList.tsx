  ...
  const renderItem = ({item, index}) => (
    <View style={styles.itemContainer}>
      ...
      <View style={styles.buttonContainer}>
        <Button
          title={translate('IMPS')}
          onPress={() => onImpsPress(item)}
          color="#007bff"
        />
        <Button
          title={translate('NEFT')}
          onPress={() => onNeftPress(item)}
          color="#28a745"
        />
        <Button
        ...
        <TouchableOpacity>
          {item.isLoading ? (
            ...
            <Button
              ...
              onPress={() => onVerifyPress(item, index)}
              color="#17a2b8"
            />
          )}
        </TouchableOpacity>
...
const BanificaryBanklist = (bankList: any) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{translate('Beneficiary_List')}</Text>
      <BeneficiaryList
        bankList={bankList}
