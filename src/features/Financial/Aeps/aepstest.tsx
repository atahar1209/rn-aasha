...
import {Alert, View} from 'react-native'; // Import Text from react-native
...
const TwoFaComponent = () => {
  const {userId, activeAepsLine} = useSelector(
  ...
  const TwoFa = async () => {
    ...
    try {
      ...
      Alert.alert(
        translate('Error'),
        `${translate('An error occurred')}: ${error.message}`,
        [{text: translate('OK'), onPress: () => console.log('OK Pressed')}],
        {cancelable: false},
      );
    }
  ...
  const onPressLogin = useCallback(
    async (captureResponse1, cardnumberORUID1) => {
      ...
      Alert.alert(translate('Message'), `: ${response.Message}`);
    },
    [latitude, longitude, userId, post, activeAepsLine],
  );
  useEffect(() => {}, []);
    ...
    async (captureResponse1, cardnumberORUID1) => {
      ...
      try {
        const response = await fetch(
          'http://api.divyanshipay.co.in//AEPS/api/app/AEPS/balanceEnquiry',
          {
            method: 'POST',
            headers: headers,
        ...
        if (!response.ok) {
          throw new Error(
            translate('Network response was not ok') + response.statusText,
          );
        }
        ...
        if (responseData.RESULT === '1') {
          Alert.alert(translate('Error'), `: ${responseData.ADDINFO}`);
