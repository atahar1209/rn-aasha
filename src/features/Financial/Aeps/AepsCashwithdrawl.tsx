...
const AepsCW = () => {
  ...
  const {userId, Loc_Data, activeAepsLine} = useSelector(
  ...
  useEffect(() => {
    ...
    if (fingerprintData?.PidData?.Resp?.errCode === 0 && isScan) {
      // OnPressEnq();
    }
  ...
  const OnPressEnq2 = async fingerprintData => {
    try {
      ...
      if (!pidData) throw new Error(translate('Invalid PID Data'));
  ...
  const openFace = () => {
    openFaceAuth(userId)
      ...
      .catch(async error => {
        ...
        Alert.alert(translate('Face authentication failed'));
      });
  };
  ...
  const videostatus = async () => {
    try {
      const response = await get({
        url: activeAepsLine
          ? `${APP_URLS.adharpayvideostatusNifi}`
  ...
  const OnPressEnq = async (fingerprintDataString, pidDataXml) => {
    try {
      const parsedJson =
        typeof fingerprintDataString === 'string'
  ...
  const BEnQ = useCallback(
    async (captureResponse1, cardnumberORUID1, pidDataX, isface) => {
      ...
      if (provider === 'NIFI') {
        finalUrl = 'AEPS/api/Nifi/app/Aeps/cashWithdrawal';
      } else if (provider === 'CHAGANS') {
        finalUrl = 'AEPS/api/Chagan/data/cashWithdrawal';
      } else if (provider === 'FINGPAY') {
        finalUrl = 'AEPS/api/app/Aeps/cashWithdrawal';
      }
  ...
  const handleAadhaarScan = scannedData => {
    ...
    if (obj.uid || obj.name) {
      ...
      Alert.alert(
        translate('Success'),
        `${translate('Aadhaar Found')}: ${obj.name}`,
      );
    } else {
  ...
  const CheckEkyc = async () => {
    ...
    try {
      ...
      Alert.alert(
        translate('Error'),
        translate('Something went wrong while checking status.'),
      );
    } finally {
  ...
  if (isScan2) {
    ...
    if (device == null)
      return <Text>{translate('No_Camera_Device_Found')}</Text>;
