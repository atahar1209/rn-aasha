...
const AepsCW = () => {
  ...
  const {userId, Loc_Data, activeAepsLine, isfaceScan} = useSelector(
  ...
  const getUserNamefunction = useCallback(
    async MoNumber => {
      setIsLoading(true);
      try {
        const response = await get({
          url: activeAepsLine
            ? `${APP_URLS.aepsNameinfoNifi}${MoNumber}`
  ...
  const aepsresponsepress = useCallback(
    (addinfo: {
  ...
  const checkvideo = useCallback(
    async (transamount, agentid, AddharpayResponse) => {
      try {
  ...
  const adhar_Validation = useCallback(
    async adharnumber => {
      setIsLoading(true);
  ...
  const BEnQ = useCallback(
    async (
      captureResponse1: any,
      cardnumberORUID1: any,
      pidDataX: any,
      isface: any,
    ) => {
      ToastAndroid.show(
        String(isface ? translate('ok') : translate('OK')),
        ToastAndroid.SHORT,
      );
      setIsLoading(true);
        ...
        if (RESULT === '0') {
          ...
          if (isface) {
            Dialog.show({
              ...
              title: translate('cashWithdrawal'),
              textBody: ADDINFO || translate('Unknown error occurred'),
            ...
            Alert.alert(
              isface ? translate('cashWithdrawal-F') : translate('CW-NF'),
              ADDINFO,
            );
          }
        }
  ...
  const OnPressEnq = useCallback(
    async (
      ...
      if (!fingerprintData || !fingerprintData['PidData']) {
        Alert.alert(
          translate('Error'),
          translate('Invalid fingerprint data. Please try again.'),
      ...
      if (!aadharNumber) {
        Alert.alert(
          translate('Error'),
          translate('Aadhar number and Bank ID are required.'),
          [{text: translate('OK'), onPress: () => {}}],
        );
        setIsLoading(false);
        return;
      ...
      const captureResponse = {
        Devicesrno:
          fingerprintData['PidData']['DeviceInfo'].additional_info.Param[0][
            'value'
          ],
        PidDatatype: 'X',
      ...
      try {
        ...
        Alert.alert(
          translate('Error'),
          translate(
            'There was an issue submitting the fingerprint data. Please try again later.',
          ),
          [{text: translate('OK'), onPress: () => {}}],
        );
      }
  ...
  const findIsFacialTan = useCallback(
    iINNo => {
      console.log(iINNo);
  ...
  const handleServifeeChange = useCallback(
    text => setServifee(text),
    [setServifee],
  );
  ...
  const handleConsumerNameChange = useCallback(
    text => {
      ...
      <ScrollView
        ...
        <View style={styles.container}>
          <View style={styles.body}>
            <TouchableOpacity style={{}} onPress={handleBankPress}>
              <>
                <FlotingInput
                  editable={false}
