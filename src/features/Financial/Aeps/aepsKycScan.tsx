...
const Aepsekycscan = () => {
  ...
  const {aadharNumber, setFingerprintData, Loc_Data, userId} =
  ...
  const BEnQ = useCallback(
    async (captureResponse1, cardnumberORUID1, pidDataX, piddata) => {
      setIsLoading(true);
      try {
        ...
        if (provider === 'NIFI') {
          finalUrl = APP_URLS.AepsKycFinScanNifi;
        } else if (provider === 'CHAGANS') {
          finalUrl = 'AEPS/api/Chagan/data/EkycVerifyFinger';
        } else if (provider === 'FINGPAY') {
          finalUrl = APP_URLS.AepsKycFinScan;
        }
          ...
          [
            {
              ...
              onPress: () =>
                navigation?.navigate(
                  response.Status === true
                    ? translate('AepsTabScreen')
                    : translate('ReportScreen'),
                ),
            },
          ],
        );
  ...
  const handleSelection = selectedOption => {
    if (deviceName === 'Device' || !deviceName) {
      return Alert.alert(
        ...
        translate('Please select a device.'),
      );
    }
    ...
    if (selectedCapture) {
      isDriverFound(selectedCapture)
        .then(() => capture(selectedCapture))
        .catch(() =>
          Alert.alert(
            translate('Error'),
            translate('RD Service Driver Not Found'),
          ),
        );
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View
          ...
          <Text style={[styles.title, {color: isDarkMode ? '#FFF' : '#333'}]}>
            {translate('E-KYC is Not Completed')}
          </Text>
          <Text style={styles.subtitle}>
            {translate(
              'Please complete your verification to enable all AEPS features.',
            )}
          </Text>
        </View>
        ...
        <View
          ...
          <Text
            style={[styles.cardTitle, {color: isDarkMode ? '#CCC' : '#666'}]}>
            {translate('Follow these steps')}:
          </Text>

          <View style={styles.stepRow}>
            <View style={styles.dot} />
            <Text
              style={[styles.stepText, {color: isDarkMode ? '#BBB' : '#444'}]}>
              {translate(
                'Connect your Biometric Device (Mantra, Morpho, etc.) via OTG.',
              )}
            </Text>
          </View>
          <View style={styles.stepRow}>
            ...
            <Text
              style={[styles.stepText, {color: isDarkMode ? '#BBB' : '#444'}]}>
              {translate('Select your device model from the dropdown below.')}
            </Text>
          </View>
          <View style={styles.stepRow}>
            <View style={styles.dot} />
            <Text
              style={[styles.stepText, {color: isDarkMode ? '#BBB' : '#444'}]}>
              {translate(
                'Click Scan Finger and place your finger on the scanner.',
              )}
            </Text>
          </View>
        </View>
