...
const AdharPay = ({}) => {
    ...
    async (
      ...
      try {
        ...
        const response = await post({
          url: activeAepsLine
            ? 'AEPS/api/Nifi/app/Aeps/Aadharpay'
            : 'AEPS/api/app/Aeps/Aadharpay',
          data: data,
          config: {
            headers,
          },
        ...
        if (RESULT === '0') {
          ...
          setIsLoading(false);
          Alert.alert(translate(`Note:`), ADDINFO);
        }
      } catch (error) {
    ...
    try {
      const granted = await PermissionsAndroid.request(
        PermissionsAndroid.PERMISSIONS.CAMERA,
        {
          ...
          message: translate('key_thisappn_98'),
  ...
  async function adhar_Validation(adharnumber) {
    try {
      const response = await get({
        url: activeAepsLine
          ? `/Common/api/data/AdharCardValidationCheck?aadharnumber=${adharnumber}`
  ...
  const checkvideo = async (transamount, agentid, AddharpayResponse) => {
    try {
      const response = await post({
        url: activeAepsLine
          ? `${APP_URLS.checkadharpayvideoNifi}?transamount=${transamount}&agentid=${agentid}`
  ...
  const uploadvideo = async (video, Agentid) => {
    try {
      const response = await post({
        url: activeAepsLine
          ? `${APP_URLS.checkadharpayvideoNifi}Videolink=${video}&agentid=${Agentid}`
  ...
  const videostatus = async () => {
    try {
      const response = await get({
        url: activeAepsLine
          ? `AEPS/api/Nifi/data/VideoStatus`
          : `${APP_URLS.adharpayvideostatus}`,
      });
      if (response.Status === true) {
        setShowDialog(false);
      } else {
  ...
  const sendOtp = async () => {
    try {
      const response = await post({
        url: activeAepsLine
          ? `${APP_URLS.adharpaysendotpNifi}amount=${amountcont}&usermobile=${mobileNumber}`
          : `${APP_URLS.adharpaysendotp}amount=${amountcont}&usermobile=${mobileNumber}`,
      });
      if (response.status === '1') {
        ToastAndroid.showWithGravity(
          `${translate('sending otp failed')},${response.ADDINFO}`,
          ToastAndroid.SHORT,
          ToastAndroid.BOTTOM,
        );
      } else {
        ...
        <View style={styles.container}>
          <View style={styles.dialog}>
            ...
            <Text style={styles.detailText}>
              {translate('Transaction Status')}: {status}
            </Text>
            <Text style={styles.detailText}>
              {translate('Bank RRN')}: {bnkrrn}
            </Text>
            <Text style={styles.detailText}>
              {translate('Date & Time')}: {date}
            </Text>
            <Text style={styles.detailText}>
              {translate('Amount')}: ₹{amount}
            </Text>
            <Text style={styles.detailText}>
              {translate('Aadhar Number')}: {aadharNumber}
            </Text>

            <TouchableOpacity style={styles.button} onPress={handleUploadVideo}>
              <Text style={styles.buttonText}>
                {translate('Upload_Video_Now')}
              </Text>
            </TouchableOpacity>
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                {translate('Aadhar_Pay_Receipt')}
              </Text>
            </View>
            <View style={styles.modalContent}>
              <Text style={styles.contentText}>
                {translate('Transaction Status')}: {trnstatus}
              </Text>
              <Text style={styles.contentText}>
                {translate('Bank RRN')}: {trnbnkrrn}
              </Text>
              <Text style={styles.contentText}>
                {translate('Date & Time')}: {datee}
              </Text>
              <Text style={styles.contentText}>
                {translate('Amount')}: ₹{trnsamount}
              </Text>
              <Text style={styles.contentText}>
                {translate('Remain Amount')}: ₹{remamount}
              </Text>
              <Text style={styles.contentText}>
    ...
    return (
      <View
        ...
        {/* <View style={{
                    ...
                    <CloseAadharSvg />
                    <Text style={{ fontSize: 24, fontWeight: 'bold', marginBottom: 15, color: colorConfig.primaryColor }}>{translate("Service_Disabled")}</Text>
                    <View style={{ padding: 10 }}>
                        <Text style={{ fontSize: 16, color: '#333', marginBottom: 5 }}>{translate("key_asperthe_140")}</Text>
                        <Text style={{ fontSize: 16, color: '#333' }}>{translate("key_aadhaarpa_141")}</Text>
                    </View>
                </View> */}
        <NotFoundScreen
          title2={'Service Disabled'}
          description={`As per the norms, Aadhaar Pay service cannot be used with Wallet service.${'\n'}Aadhaar Pay service cannot be used with Wallet service.`}
          buttontitle={translate('Go Back')}
        />
      </View>
    );
  }
 ...
 */}
      ...
      <BankBottomSite
        ...
        setisbank={setisbank}
        onPress1={function (id: string): void {
          throw new Error(translate('Function not implemented.'));
        }}
        setisFacialTan={function (value: boolean): void {
          throw new Error(translate('Function not implemented.'));
        }}
      />
    </View>
