...
const SERVICES = [
  {
    ...
    subtitle: translate('Withdraw cash via Aadhaar'),
    icon: () => <AadharPaysvg />,
    barColor: '#4CAF50',
  },
  {
    ...
    subtitle: translate('Check account balance'),
    icon: () => <CheckBlance />,
    barColor: '#2196F3',
  },
  {
    key: 'AepsMiniStatement',
    title: `${translate('Mini')}\n${translate('Statement')}`,
    subtitle: translate('View last transactions'),
    icon: () => <StatementSvg />,
    barColor: '#FF9800',
  },
  {
    key: 'AadharPay',
    title: `${translate('Aadhaar')}\n${translate('Pay')}`,
    subtitle: translate('Pay using Aadhaar'),
    icon: () => <Aeps />,
    barColor: '#9C27B0',
  },
];
...
const AepsTabScreen = () => {
  ...
  const RegisterMerchant = useCallback(async () => {
    try {
      ...
      if (
        ...
        Alert.alert(
          translate('Success'),
          translate('Merchant Registered Successfully'),
        );
      ...
      Alert.alert(
        translate('Notice'),
        response?.Message || translate('Registration Failed'),
      );
    } catch (e: any) {
      ...
      Alert.alert(
        translate('API Error'),
        e?.message || translate('Something went wrong'),
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  const CheckEkyc = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(translate('Server Error (404/500)'));
      }
      ...
      if (status === true || message === translate('DONE')) {
        await CheckAeps();
        return;
      }
      ...
      Alert.alert(
        ...
        message || translate('Unknown Status'),
      ...
      Alert.alert(
        translate('API Error'),
        e?.message || translate('Internal Server Error'),
      );
    } finally {
      isApiCalling.current = false;
      setIsProcessing(false);
    }
  }, [activeAepsLine?.provider, get, CheckAeps, navigation]);
  ...
  }, [activeAepsLine, CheckEkyc, UserStatus]);
    ...
    <View style={styles.header}>
      ...
      <View style={[styles.lineChip, {borderColor: themeColor}]}>
        ...
        <Text style={[styles.lineLabel, {color: themeColor}]}>
          {activeAepsLine ? translate('Standard') : translate('Standard')}
        </Text>
      </View>
    </View>
  );
    ...
    return (
      <View style={[styles.root, {backgroundColor: color1}]}>
        <GlassHeader
          ...
          subtitle={translate('AEPS Services')}
    ...
    <AepsContext.Provider value={contextValue}>
      ...
      {activeService ? (
        ...
        <View style={[styles.root, {backgroundColor: color1}]}>
          ...
          <GlassHeader
            title={translate('AEPS Services')}
            subtitle={translate('Aadhaar Enabled Payment System')}
          ...
          {isProcessing ? (
            <View style={styles.loaderWrap}>
              ...
              <Text style={styles.loaderText}>
                {translate('checking_status') || translate('Checking Status')}
              </Text>
            </View>
          ) : UserStatus === 'Success' ? (
            <ScrollView
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.modalCard}>
                ...
                <Text style={[styles.modalTitle, {color: themeColor}]}>
                  {translate('Required') || translate('Action Required')}
                </Text>
                ...
                <View style={styles.modalRow}>
                  ...
                  <TouchableOpacity
                    ...
                    <Text style={styles.modalPrimaryText}>
                      {translate('Complete_eKYC') || translate('Complete eKYC')}
                    </Text>
                  </TouchableOpacity>
                </View>
              </View>
