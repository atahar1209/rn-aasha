...
const AepsMinistatement = () => {
  ...
  const BEnQ = useCallback(
    async (captureResponse1, cardnumberORUID1, pidDataX, isface) => {
      ...
      if (provider === 'NIFI') {
        finalUrl = 'AEPS/api/Nifi/app/AEPS/MiniStatement';
      } else if (provider === 'CHAGANS') {
        finalUrl = 'AEPS/api/Chagan/data/MiniStatement';
      } else if (provider === 'FINGPAY') {
        finalUrl = 'AEPS/api/app/AEPS/MiniStatement';
      }
      ...
      try {
        ...
        if (RESULT && RESULT.toString() === '0') {
        ...
        } else if (RESULT && RESULT.toString() === '1') {
          Alert.alert(
            translate('Message'),
          ...
          Alert.alert(
            translate('Message'),
            translate('Unexpected response:') + JSON.stringify(response),
          );
        }
        // // ✅ Pura response alert mein
        // Alert.alert(
        //     '📦 API Response',
        ...
        //                     );
        //                 } else {
        //                     Alert.alert('Message', 'Unexpected response: ' + JSON.stringify(response));
        //                 }
        //             },
        //         },
        //     ]
        ...
        Alert.alert(
          translate('Error'),
          translate('Error during Mini Statement:') + String(error),
        );
      }
    },
    [
  ...
  return (
    <View style={styles.main}>
      {/* कीबोर्ड को हैंडल करने के लिए KeyboardAwareScrollView का उपयोग */}
      <KeyboardAwareScrollView
        ...
        <View style={styles.container}>
          ...
          <TouchableOpacity
            ...
            <FlotingInput
              editable={false}
              label={bankName ? bankName.toString() : translate('Select Bank')}
              placeholder={bankName ? '' : translate('Select Bank')}
      ...
      {/* Bank Bottom Selection Sheet (Keep outside ScrollView) */}
      {isFocused && (
        <BankBottomSite
          setBankId={setBankId}
          bankdata={banklist}
