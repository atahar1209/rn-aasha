...
const Rechargedetails = ({route}) => {
  ...
  useEffect(() => {
    ...
    console.log(IsRington, status, Message);
    const mockNotification = {
      notification: {
        title: Message || translate('Status Unknown'),
        body: `${translate('Recharge of')} ${
          mobileNumber ?? translate('N/A')
        }. ${translate('Amount')}: ${Amount ?? '0'} is ${
          status ?? translate('Unknown')
        }. ${translate('Transaction ID')}: ${
          idno ?? translate('N/A')
        }. ${translate('Regards')} ${
          APP_URLS?.AppName ?? translate('YourApp')
        }`,
      },
    };
          ...
          <ViewShot
            ...
            <View style={[styles.container]}>
              <Text style={[styles.title, {color: colorConfig.primaryColor}]}>
                {translate('Transaction_Information')}
              </Text>
