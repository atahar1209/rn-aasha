...
const CableTvScreen = () => {
  ...
  const [accmaxlength2, setAccmaxlength2] = useState();
  ...
  async function CableTvOpt(opttype) {
    try {
  ...
  async function billInfo() {
    try {
      ...
      const url = `${APP_URLS.rechargeViewBil}billnumber=${CustomerID}&Operator=${optcode}&billunit=${accnumhint}&ProcessingCycle&acno=&lt=&ViewBill="Y"`;
  ...
  const validateFields = () => {
    ...
    } else if (!amount || amount === 'Enter Amount' || amount === 0) {
      ToastAndroid.showWithGravity(
  ...
  return (
    <View style={styles.main}>
      <AppBarSecond title={'Cable TV Screen'} />

      {/* कीबोर्ड मैनेजमेंट के लिए KeyboardAwareScrollView का उपयोग */}
      <KeyboardAwareScrollView
        keyboardShouldPersistTaps={'handled'}
        enableOnAndroid={true}
        extraScrollHeight={100} // कीबोर्ड और बटन के बीच गैप रखने के लिए
        style={{flex: 1}}
        ...
        <View style={styles.container}>
          ...
          {/* Consumer Number & Info Logic */}
          <View>
            <FlotingInput
              label={paramname}
              value={consumerNo}
              onChangeTextCallback={text => {
                const cleanedText = text.replace(/\D/g, ''); // सिर्फ नंबर्स रखने के लिए
      ...
      {/* Modals & BottomSheets (इन्हें ScrollView के बाहर रखना बेहतर है) */}
      <RecentHistory
      ...
      <Rechargeconfirm
        Lottieimg={require('../../utils/lottieIcons/plug.json')}
