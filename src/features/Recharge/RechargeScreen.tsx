...
const RechargeScreen = () => {
  ...
  const Rechargepin = async () => {
    try {
      const url = `${APP_URLS.RechargePin}`;
      console.log(url);
  ...
  async function getopertaorlist(op: any) {
    try {
      ...
      const hh = await post({
        url: `${
          APP_URLS.plandetails
        }optname=${operator}&circlename=${circle}&type=${'mobile'}`,
      });
      console.log('hh', hh);
  ...
  const getOperator = useCallback(async (mo: any) => {
    ...
    try {
      ...
      if (response.status !== 'Failed') {
        ...
        if (ispost) {
          setSelectedOption(translate('RechargeScreen.Postpaid'));
        } else {
          setSelectedOption(translate('RechargeScreen.prepaid'));
        }
      } else {
  ...
  const openContactPicker = useCallback(async () => {
    try {
      ...
      // Native module sends code "CANCELLED" on back-press / cancel
  ...
  const onRechargePress = useCallback(async () => {
    ...
    try {
      ...
      const url = `${
        APP_URLS.rechTask
      }rd=${rd}&n=${n1}&ok=${ok1}&amn=${Amount}&pc=&bu=&acno=&lt=&ip=${ip1}&mc=&em=${Model}&offerprice=&commAmount=&Devicetoken=${devtoken}&Latitude=${Latitude1}&Longitude=${Longitude1}&ModelNo=${ModelNo}&City=${devtoken}&PostalCode=${PostalCode}&InternetTYPE=${InternetTYPE}&Addresss=${Addresss}&value1=${encodeURIComponent(
    ...
    <View style={styles.main}>
      ...
      <View style={styles.container}>
            ...
            onChangeTextCallback={text => {
              if (text.length === 10) {
                if (validateMobileNumber(text)) {
                  ...
                  ToastAndroid.showWithGravity(
                    translate(
                      'RechargeScreen.Please Enter Vaild Mobile Number',
                    ),
                    ToastAndroid.SHORT,
                    ToastAndroid.BOTTOM,
                  );
        ...
        <Rechargeconfirm
          ...
          details={[
            {
              label: translate("RechargeScreen.Recharge Type"),
              value2: ispost
                ? translate("RechargeScreen.Postpaid")
                : translate("RechargeScreen.Prepaid"),
            },
            { label: translate("RechargeScreen.Mobile"), value: mobileNumber },
            {
              label: translate("RechargeScreen.Operator Name"),
              value2: operator,
            },
          ]}
          lastlabel={translate("RechargeScreen.Transaction Amount")}
          lastvalue={Amount}
          onRechargedetails={() => {
            onRechargePress();
