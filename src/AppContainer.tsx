...
export const AppContainer = () => {
  ...
  try {
    ...
    initializeApp(firebaseConfig, 'aircharge');
  }
  ...
  const renderMainContent = () => {
    ...
    if (APP_URLS.AppName === 'Maxus Pay') {
      if (!update && allowed === true) {
        return (
          <Updatebox isVer={undefined} loading={undefined} isplay={false} />
        );
      }
    }
    if (!update) {
  ...
  return <SafeWrapper>{renderMainContent()}</SafeWrapper>;
};
