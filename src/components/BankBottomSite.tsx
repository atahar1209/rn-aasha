...
type BankBottomSiteProps = {
  isbank: boolean;
  setisbank: (value: boolean) => void;
  setBankName: (name: string) => void;
  bankdata: BankDataItem[];
  setBankId: (id: string) => void;
  onPress1: (id: string) => void;
  setisFacialTan: (value: boolean) => void;
};

const BankBottomSite: React.FC<BankBottomSiteProps> = ({
  isbank,
  setisbank,
  setBankName,
  bankdata,
    ...
    <TouchableOpacity
      ...
      {item.isFacialTan && (
        <View
          ...
          <FacescanSvg />
          <Text style={itemS.faceText}>{translate('Face')}</Text>
        </View>
      )}
  ...
  const EmptyList = () => (
    <View style={emptyS.wrap}>
      ...
      <Text style={emptyS.title}>{translate('No_banks_found')}</Text>
      <Text style={emptyS.sub}>{translate('Try_a_different_search_term')}</Text>
    </View>
  );
    ...
    <BottomSheet
      ...
      <View style={sheet.container}>
        ...
        <View
          style={[
            searchS.wrap,
            {borderColor: searchQuery.length > 0 ? PRIMARY : '#E2E8F0'},
          ]}>
          <Text
            style={[
              searchS.icon,
              {color: searchQuery.length > 0 ? PRIMARY : '#94A3B8'},
            ]}>
            ⌕
          </Text>
          <TextInput
