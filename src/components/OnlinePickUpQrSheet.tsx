...
interface OnlinePickUpQrSheetProps {
  isVisible: boolean;
...
const OnlinePickUpQrSheet = ({
  ...
  const handleUtrSubmit = useCallback(() => {
    ...
    if (!utr || utr.length < 12) {
      ToastAndroid.show(
        translate('UTR must be at least 12 characters'),
        ToastAndroid.SHORT,
      );
      return;
    }
    ...
    Alert.alert(
      translate('Verify UTR'),
      `${translate('You entered UTR is')}:\n\n${utr}\n\n${translate(
  ...
  }, [currentPreviewImageRef, onUpload, setIsVisible]);
  return (
        ...
        <View style={styles.sheetContent}>
          ...
          <LinearGradient
            ...
            <Text style={styles.title}>
              {translate('State Bank Of India')} ₹{am}.00
            </Text>
          ...
          {vpa ? (
            <View style={{marginTop: 16, alignItems: 'center'}}>
              <Text style={styles.payeeText}>
                {translate('Payee')}: {payee}
              </Text>
              <Text style={styles.vpaText}>
                {translate('VPA')}: {vpa.toLocaleUpperCase()}
              </Text>

              {utr.length >= 12 && (
                <Text style={styles.payeeText}>
                  {translate('UTR No')}: {utr}
                </Text>
              )}
            </View>
          ) : null}
          ...
          {!utrSubmitted && (
            <View style={styles.inputRow}>
              <TextInput
                style={styles.input}
                placeholder={translate('Enter UTR (min 12 chars)')}
                value={utr}
                onChangeText={setUtr}
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      utr.length >= 12 ? colorConfig.primaryColor : color1,
                  },
                ]}>
                <Text style={styles.utrButtonText}>{translate('Submit')}</Text>
              </TouchableOpacity>
            </View>
          )}
