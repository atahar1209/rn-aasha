...
const WalletCardSection: React.FC<Props> = ({
  cards,
  primaryColor = '#1A237E',
}) => {
    ...
    <Animated.View
      ...
      <TouchableOpacity activeOpacity={0.92} onPress={item.onPress}>
        <LinearGradient
          ...
          {/* Logo top-left */}
          {item.logo && <View style={styles.logoWrap}>{item.logo}</View>}
      ...
      <TouchableOpacity style={styles.viewBtn} onPress={item.onPress}>
        <Text style={styles.viewBtnText}>{translate('View Details')}</Text>
      </TouchableOpacity>
    </Animated.View>
  );
  ...
  const renderStackCard = ({
    ...
    <TouchableOpacity
      ...
      <LinearGradient
        ...
        {item.logo && <View style={styles.stackLogoWrap}>{item.logo}</View>}
        <View style={styles.stackTextWrap}>
          <Text style={styles.stackTitle}>{translate(item.title)}</Text>
          {item.subtitle && (
            <Text style={styles.stackSubtitle}>{translate(item.subtitle)}</Text>
          )}
        </View>
        ...
        <View style={styles.stackRoot}>
          ...
          <View style={styles.stackHeader}>
            ...
            <TouchableOpacity onPress={() => goToCarousel(activeIdx)}>
              <Text style={styles.stackCloseBtn}>✕ {translate('Close')}</Text>
            </TouchableOpacity>
          </View>
